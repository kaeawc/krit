...
// buildSyntheticShard returns a fileShard sized like a large-project shard:
// a small symbol set and many references drawn from a Zipf-ish name
// distribution so the intra-shard name table has real de-duplication to
// exploit.
func buildSyntheticShard(symCount, refCount int) *fileShard {
	...
	path := "/workspace/app/src/main/kotlin/com/example/database/MessageTable.kt"
	s := &fileShard{
		Path:        path,
...
// time per shard. This is what every cache miss pays on write.
