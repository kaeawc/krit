...
func RunNewExperimentScaffold(opts NewExperimentOpts) int {
	if code := validateExperimentOpts(&opts); code != 0 {
		return code
	}
	...
	wireAbs, relFromRoot, code := resolveWireFile(opts.WireFile, repoRoot)
	if code != 0 {
		return code
	}

	importAdded, code := updateExperimentFiles(opts, repoRoot, wireAbs)
	if code != 0 {
		return code
	}

	printScaffoldResult(opts, relFromRoot, importAdded)
...
func validateExperimentOpts(opts *NewExperimentOpts) int {
	if opts.Name == "" {
...
func resolveWireFile(wireFile, repoRoot string) (wireAbs, relFromRoot string, code int) {
	wireAbs = wireFile
	if !filepath.IsAbs(wireAbs) {
		wireAbs = filepath.Join(repoRoot, wireFile)
	}
	...
	rel, err := filepath.Rel(repoRoot, wireAbs)
	...
	return wireAbs, rel, 0
}

func updateExperimentFiles(opts NewExperimentOpts, repoRoot, wireAbs string) (importAdded bool, code int) {
...
func printScaffoldResult(opts NewExperimentOpts, relFromRoot string, importAdded bool) {
