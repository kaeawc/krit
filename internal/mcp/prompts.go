...
func promptDefinitions() []PromptDefinition {
	return []PromptDefinition{
		{
			Name:        "review_kotlin",
			Description: "Review Kotlin code for issues and suggest improvements. Runs analyze + fix.suggest then formats results as a code review.",
			Arguments: []PromptArgument{
				{Name: "code", Description: "Kotlin source code to review", Required: true},
				{Name: "path", Description: "File path (for context like package detection)", Required: false},
			},
		},
		{
			Name:        "prepare_pr",
			Description: "Analyze project changes for PR readiness. Runs analyze.project and summarizes findings suitable for a PR description.",
			Arguments: []PromptArgument{
				{Name: "paths", Description: "Comma-separated list of directories or files to analyze", Required: true},
			},
		},
		{
			Name:        "refactor_check",
			...
			Arguments: []PromptArgument{
				{Name: "symbol", Description: "Symbol name to check references for", Required: true},
				{Name: "code", Description: "Kotlin source code containing the symbol", Required: false},
				{Name: "project_paths", Description: "Comma-separated list of directories to search for references", Required: false},
			},
		},
	}
}
...
// promptReviewKotlin runs analyze + fix.suggest and formats as a code review.
func (s *Server) promptReviewKotlin(arguments map[string]string) (*PromptGetResult, *RPCError) {
	...
	// analyze.code
	analyzeArgsJSON, _ := json.Marshal(analyzeArgs{Mode: "code", Code: code, Path: path})
	analyzeResult := s.toolAnalyze(analyzeArgsJSON)

	// fix.suggest
	fixArgsJSON, _ := json.Marshal(fixArgs{Operation: "suggest", Code: code, Path: path, FixLevel: "all"})
	fixResult := s.toolFix(fixArgsJSON)
...
// promptPreparePR runs analyze.project and formats findings for a PR description.
func (s *Server) promptPreparePR(arguments map[string]string) (*PromptGetResult, *RPCError) {
	...
	projectArgsJSON, _ := json.Marshal(analyzeArgs{Mode: "project", Paths: paths, Format: "summary"})
	projectResult := s.toolAnalyze(projectArgsJSON)
...
// promptRefactorCheck runs symbols.references and optionally types queries.
func (s *Server) promptRefactorCheck(arguments map[string]string) (*PromptGetResult, *RPCError) {
	...
	var sb strings.Builder
	fmt.Fprintf(&sb, "You are checking whether it is safe to refactor the symbol '%s'. ", symbol)
	...
	if projectPathsStr != "" {
		...
		refArgs, _ := json.Marshal(symbolsArgs{
		...
		refResult := s.toolSymbols(refArgs)
	...
	// types.classes + types.hierarchy
	code := arguments["code"]
	if code != "" {
		typesArgs1, _ := json.Marshal(typesArgs{Code: code, Query: "classes"})
		typesResult := s.toolTypes(typesArgs1)
		...
		hierarchyArgsJSON, _ := json.Marshal(typesArgs{Code: code, Query: "hierarchy"})
		hierarchyResult := s.toolTypes(hierarchyArgsJSON)
