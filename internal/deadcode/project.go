...
func AnalyzeProject(scanRoot string, opts ProjectOptions) ([]ProjectFinding, error) {
	...
	graph, idx, fileByPath, err := buildProjectIndex(scanRoot, paths, opts.Workers)
	if err != nil {
		return nil, err
	}

	rootNames := collectRootNames(scanRoot, opts.Roots, graph)
	nameToSyms, fileRefs := buildSymbolMaps(idx)
	reachable := markReachable(idx, fileByPath, rootNames, graph, nameToSyms, fileRefs)
	findings := collectFindings(idx, reachable, graph)
...
func buildProjectIndex(scanRoot string, paths []string, workers int) (*module.Graph, *scanner.CodeIndex, map[string]*scanner.File, error) {
	...
	return graph, idx, fileByPath, nil
}

func collectRootNames(scanRoot string, extraRoots []string, graph *module.Graph) map[string]bool {
	rootNames := buildSimpleNameSet(extraRoots)
	if graph != nil {
...
func buildSymbolMaps(idx *scanner.CodeIndex) (nameToSyms map[string][]int, fileRefs map[string]map[string]bool) {
	...
	return nameToSyms, fileRefs
}

func markReachable(idx *scanner.CodeIndex, fileByPath map[string]*scanner.File, rootNames map[string]bool, graph *module.Graph, nameToSyms map[string][]int, fileRefs map[string]map[string]bool) []bool {
...
func collectFindings(idx *scanner.CodeIndex, reachable []bool, graph *module.Graph) []ProjectFinding {
...
func isRootSymbol(s scanner.Symbol, f *scanner.File, rootNames map[string]bool, graph *module.Graph) bool {
