...
func BuildGraph(files []*scanner.File, moduleGraph *module.Graph) *Graph {
	graph := &Graph{
...
func buildBindingSeedFlat(idx uint32, file *scanner.File, pkg string, imports map[string]string, modulePath string) (bindingSeed, bool) {
	...
	file.FlatWalkNodes(ctor, "class_parameter", func(param uint32) {
		...
		seed.dependencies = append(seed.dependencies, pendingDependency{
			...
			typeName:      typeName,
			deferred:      dependencyIsDeferred(file.FlatNodeText(typeNode)),
		})
	})
...
func dependencyIsDeferred(text string) bool {
	text = strings.TrimSpace(strings.TrimSuffix(text, "?"))
	outer, _, ok := splitOuterAndFirstArg(text)
	if !ok {
		return false
	}
	return dependencyWrappers[outer]
}
