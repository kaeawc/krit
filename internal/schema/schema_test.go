...
func TestGenerateSchema_HasAllRuleSets(t *testing.T) {
	...
	for set := range knownSets {
		if _, ok := schema.Properties[set]; !ok {
...
func TestGenerateSchema_HasConfigurableRules(t *testing.T) {
	...
	complexity, ok := schema.Properties["complexity"]
	if !ok {
		t.Fatal("schema missing 'complexity'")
	}
	longMethod, ok := complexity.Properties["LongMethod"]
	if !ok {
		t.Fatal("complexity missing 'LongMethod'")
	}
	if _, ok := longMethod.Properties["allowedLines"]; !ok {
...
func TestGenerateSchema_HasModuleTemplate(t *testing.T) {
	...
	if !ok {
		t.Fatal("schema missing module_template")
	}
	for _, key := range []string{"feature_root", "required_submodules", "required_plugins"} {
		if _, ok := moduleTemplate.Properties[key]; !ok {
			t.Fatalf("module_template schema missing %s", key)
		}
	}
}

func TestGenerateSchema_HasSLOs(t *testing.T) {
	schema := GenerateSchema(CollectRuleMeta())
	slos, ok := schema.Properties["slos"]
	if !ok {
		t.Fatal("schema missing slos")
	}
	if slos.Items == nil {
		t.Fatal("slos missing items")
	}
	for _, key := range []string{"module", "max_warnings_per_kloc", "max_errors_per_kloc"} {
		if _, ok := slos.Items.Properties[key]; !ok {
			t.Fatalf("slos schema missing %s", key)
		}
	}
}

func TestGenerateSchema_HasTestSourcePaths(t *testing.T) {
	schema := GenerateSchema(CollectRuleMeta())
	for _, key := range []string{"testSourcePaths", "testSourcePathsOverride"} {
		prop, ok := schema.Properties[key]
		if !ok {
			t.Fatalf("schema missing %s", key)
		}
		if prop.Type != "array" {
			t.Fatalf("%s type = %v, want array", key, prop.Type)
		}
	}
}
...
func TestValidateConfig_ValidConfig(t *testing.T) {
	...
	cfg.Data()["module_template"] = map[string]interface{}{
		"feature_root":        "feature:*",
		"required_submodules": []interface{}{"ui", "data", "domain"},
		"required_plugins":    []interface{}{"com.android.library"},
	}
	cfg.Data()["slos"] = []interface{}{
		map[string]interface{}{
			"module":                ":core",
			"max_warnings_per_kloc": 5,
			"max_errors_per_kloc":   0.0,
		},
	}
	cfg.Data()["testSourcePaths"] = []interface{}{"/src/checks/"}
	cfg.Data()["testSourcePathsOverride"] = []interface{}{"/src/verify/"}
...
func TestValidateConfig_TestSourcePathsWrongType(t *testing.T) {
	cfg := config.NewConfig()
	cfg.Data()["testSourcePaths"] = 42
	...
	if !found {
		t.Error("expected type error for testSourcePaths")
	}
}

func TestValidateConfig_SLOsWrongType(t *testing.T) {
	cfg := config.NewConfig()
	cfg.Data()["slos"] = []interface{}{
		map[string]interface{}{
			"module":                ":core",
			"max_warnings_per_kloc": "five",
		},
		map[string]interface{}{
	...
	for _, e := range errs {
		switch e.Path {
		case "slos[0].max_warnings_per_kloc":
			hasWrongType = e.Level == "error"
		case "slos[1].module":
			hasMissingModule = e.Level == "error"
		}
	}
	if !hasWrongType {
		t.Error("expected type error for slos[0].max_warnings_per_kloc")
	}
	if !hasMissingModule {
		t.Error("expected missing module error for slos[1].module")
	}
}

func TestValidateConfig_ModuleTemplateWrongType(t *testing.T) {
	cfg := config.NewConfig()
	cfg.Data()["module_template"] = map[string]interface{}{
		"required_submodules": 42,
	}

	errs := ValidateConfig(cfg)
	found := false
	for _, e := range errs {
		if e.Path == "module_template.required_submodules" && e.Level == "error" {
			found = true
			break
		}
	}
	if !found {
		t.Error("expected type error for module_template.required_submodules")
	}
}

func TestValidateConfig_ModuleTemplateMustBeObject(t *testing.T) {
	cfg := config.NewConfig()
	cfg.Data()["module_template"] = "feature:*"

	errs := ValidateConfig(cfg)
	found := false
	for _, e := range errs {
		if e.Path == "module_template" && e.Level == "error" {
			found = true
			break
		}
	}
	if !found {
		t.Error("expected type error for scalar module_template")
	}
}
...
func TestCollectRuleMeta_IncludesJavaSupportClassification(t *testing.T) {
	...
	for _, m := range metas {
		...
		if support.Status != api.LanguageSupportSupported {
			t.Fatalf("AddJavascriptInterface Java support = %s, want %s", support.Status, api.LanguageSupportSupported)
		}
		return
	}
...
func TestControversialRulesRegistry(t *testing.T) {
	...
	known := make(map[string]bool, len(api.Registry))
