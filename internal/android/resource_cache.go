...
// On-disk cache of parsed resource XML ResourceIndex results. Keyed by a
// content fingerprint over every relevant *.xml file in a single resource
// directory (absolute dir path + sorted per-file (name, content-hash)
// pairs + requested data family / mask). A warm hit skips the XML parse
// entirely and deserializes a ResourceIndex from zstd-wrapped gob.
//
...
// ResourceIndex cache. Sized generously so large multi-module repos fit in
// 128 MB.
...
func (c *ResourceIndexCache) Close() error {
	...
	if c.lru != nil {
		if removed, err := c.lru.Evict(); err != nil {
			errs = append(errs, err)
		} else if removed > 0 {
			c.evictions.Add(int64(removed))
		}
...
// CloseIdle shuts down background workers without persisting read-hit LRU
// touches. It is intended for invocations that only loaded cache entries.
...
// HasWrites reports whether this process queued or completed resource-index
// cache writes. Read-hit LRU touches do not count.
func (c *ResourceIndexCache) HasWrites() bool {
	if c == nil {
		return false
	}
	stats := c.writer.Stats()
	return stats.Queued > 0 || stats.Completed > 0 || c.lastWriteSec.Load() > 0
}
...
// layoutDirFile pairs a layout/*.xml path with its content bytes and
// precomputed resource name.
type layoutDirFile struct {
	name    string
	path    string
	content []byte
}

// drawableDirFile pairs a drawable/mipmap resource path with the resource
// name and XML content when the file can contribute selector metadata.
type drawableDirFile struct {
	name    string
	path    string
	ext     string
	content []byte
...
// computeLayoutDirFingerprint builds the per-dir cache key for one layout*
// directory. The directory path is part of the key because cached layouts
// carry absolute FilePath values and layout config qualifiers derive from the
// directory name.
func computeLayoutDirFingerprint(dir string, files []layoutDirFile) string {
	h := hashutil.Hasher().New()
	_, _ = h.Write([]byte("resource-cache-v"))
	_, _ = h.Write([]byte(resourceCacheVersionStr))
	_, _ = h.Write([]byte{0})
	_, _ = h.Write([]byte("layout"))
	_, _ = h.Write([]byte{0})
...
// computeDrawableDirFingerprint builds the per-dir cache key for one
// drawable* or mipmap* directory. ResourceIndex drawable entries depend on
// file names for all files and selector metadata for XML files, so non-XML
// content is intentionally not read or hashed.
func computeDrawableDirFingerprint(dir string, files []drawableDirFile) string {
	h := hashutil.Hasher().New()
	_, _ = h.Write([]byte("resource-cache-v"))
	_, _ = h.Write([]byte(resourceCacheVersionStr))
	_, _ = h.Write([]byte{0})
	_, _ = h.Write([]byte("drawable"))
	_, _ = h.Write([]byte{0})
	_, _ = h.Write([]byte(dir))
	_, _ = h.Write([]byte{0})
	for _, f := range files {
...
// readLayoutDirFiles lists and reads every layout/*.xml file in dir,
// returning the slice sorted by path. Files are read in parallel up to
// maxWorkers. The returned slice is suitable for both fingerprinting
// and parsing without a second read.
func readLayoutDirFiles(dir string, maxWorkers int) ([]layoutDirFile, error) {
	entries, err := os.ReadDir(dir)
	if err != nil {
		return nil, err
	}
	...
	out := make([]layoutDirFile, len(paths))
	workers := clampWorkers(maxWorkers, len(paths))
	var wg sync.WaitGroup
	sem := make(chan struct{}, workers)
	errs := make([]error, len(paths))
...
// readDrawableDirFiles lists every drawable/mipmap file in dir and reads XML
// content only. Binary drawable content does not affect ResourceIndex; names
// do, so this keeps warm fingerprinting cheap for large image directories.
func readDrawableDirFiles(dir string, maxWorkers int) ([]drawableDirFile, error) {
	entries, err := os.ReadDir(dir)
	if err != nil {
		return nil, err
	}
	...
	out := make([]drawableDirFile, len(paths))
	xmlJobs := make([]int, 0, len(paths))
	for i, p := range paths {
		base := filepath.Base(p)
		ext := filepath.Ext(base)
		out[i] = drawableDirFile{
			name: strings.TrimSuffix(base, ext),
			path: p,
			ext:  ext,
		}
		if ext == ".xml" {
			xmlJobs = append(xmlJobs, i)
		}
	}
	if len(xmlJobs) == 0 {
		return out, nil
	...
	workers := clampWorkers(maxWorkers, len(xmlJobs))
	var wg sync.WaitGroup
	sem := make(chan struct{}, workers)
	errs := make([]error, len(xmlJobs))
	for jobPos, fileIdx := range xmlJobs {
		wg.Add(1)
		sem <- struct{}{}
		go func(jobPos, fileIdx int) {
			defer wg.Done()
			defer func() { <-sem }()
			data, rerr := os.ReadFile(out[fileIdx].path)
			if rerr != nil {
				errs[jobPos] = rerr
				return
			}
			out[fileIdx].content = data
		}(jobPos, fileIdx)
	}
	wg.Wait()
	for _, e := range errs {
