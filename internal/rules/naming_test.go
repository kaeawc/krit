...
func TestVariableNaming_HonorsPrivateVariablePattern(t *testing.T) {
	...
	rule.PrivateVariablePattern = api.CompileAnchoredPattern(
...
func TestTopLevelPropertyNaming_HonorsPrivatePropertyPattern(t *testing.T) {
	...
	rule.PrivatePropertyPattern = api.CompileAnchoredPattern(
...
func TestNamingMetaRegexDefaultsMatchRegistry(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.ruleID+"/"+tc.optionName, func(t *testing.T) {
			...
			compiled := api.CompileAnchoredPattern(tc.ruleID, tc.optionName, optDefault)
			if compiled == nil {
...
func TestNaming_BooleanProperty_HonorsAllowedPattern(t *testing.T) {
	...
	rule.AllowedPattern = api.CompileAnchoredPattern(
...
func TestNaming_ConstructorParameter_HonorsPrivateParameterPattern(t *testing.T) {
	...
	rule.PrivateParameterPattern = api.CompileAnchoredPattern(
...
func TestNaming_NoNameShadowing_SeverityWarning(t *testing.T) {
	rule := buildRuleIndex()["NoNameShadowing"]
	if rule == nil {
		t.Fatal("NoNameShadowing rule is not registered")
	}
	if rule.Sev != api.Severity("warning") {
		t.Fatalf("expected NoNameShadowing registry severity warning, got %q", rule.Sev)
	}
	meta, ok := rules.MetaForRule(rule)
	if !ok {
		t.Fatal("NoNameShadowing rule has no descriptor")
	}
	if meta.Severity != "warning" {
		t.Fatalf("expected NoNameShadowing metadata severity warning, got %q", meta.Severity)
	}
}
...
func TestNaming_NoNameShadowing_DoesNotSelfSuppressOnInitializerText(t *testing.T) {
	findings := runRuleByName(t, "NoNameShadowing", `
package test
...
fun outer() {
    val foo = 1
    run {
        val foo = "NAME_SHADOWING handler info"
        println(foo)
    }
}
`)
	...
	if !saw {
		t.Fatalf("NoNameShadowing must still flag inner shadowing when the suppression keyword appears only in an initializer string literal, not in @Suppress")
	}
}
...
func TestNaming_NoNameShadowing_SkipsCollectionIterationLambdaParameters(t *testing.T) {
	findings := runRuleByName(t, "NoNameShadowing", `
package test
...
class Presenter(private val delegate: Delegate) {
    fun send(email: String, emails: List<String>, sectionDelegates: List<Delegate>) {
        emails.forEach { email ->
            println(email)
        }
        val flows = sectionDelegates.map { delegate ->
            delegate.observe()
        }
        println(flows)
    }
...
`)
	for _, f := range findings {
		if f.Rule == "NoNameShadowing" {
			t.Fatalf("NoNameShadowing should ignore collection iteration lambda parameters, got: %s", f.Message)
		}
	}
}

func TestNaming_NoNameShadowing_SkipsStandardIterationLambdaParameters(t *testing.T) {
	findings := runRuleByName(t, "NoNameShadowing", `
package test
...
fun process(
    ...
    delegate: Delegate,
    emails: List<String>,
    sectionDelegates: List<Delegate>,
    delegateFlow: Flow<Delegate>,
) {
    emails.filter { email ->
        email.isNotBlank()
    }.onEach { email ->
    ...
    sectionDelegates.flatMap { delegate ->
        listOf(delegate.observe())
    }.fold("") { delegate, value ->
        delegate + value
    }

    delegateFlow.collect { delegate ->
        println(delegate.observe())
    }
}
`)
	for _, f := range findings {
		if f.Rule == "NoNameShadowing" {
			t.Fatalf("NoNameShadowing should ignore standard iteration lambda parameters, got: %s", f.Message)
		}
	}
}

func TestNaming_NoNameShadowing_SkipsNestedScopeFunctionLambdaParameters(t *testing.T) {
	findings := runRuleByName(t, "NoNameShadowing", `
package test

class Key(val value: String)
class Binding {
    fun run(block: Binding.(Key) -> Unit) = Unit
}

fun setupUi(key: Key, binding: Binding) = with(binding) {
    run { key ->
        println(key.value)
    }
}
`)
	for _, f := range findings {
		if f.Rule == "NoNameShadowing" {
			t.Fatalf("NoNameShadowing should ignore nested scope-function lambda parameters, got: %s", f.Message)
		}
	}
}
...
func TestNaming_NoNameShadowing_SkipsLabeledThisPropertyAssignmentParameter(t *testing.T) {
	findings := runRuleByName(t, "NoNameShadowing", `
package test
...
class MessageDetailsAdapter {
    private var rowsAfterGap: List<MessageViewModel> = emptyList()

    private fun insertRowsAfterGap(
        indexToInsert: Int,
        totalToInsert: Int,
        rowsAfterGap: List<MessageViewModel>,
    ) {
        this@MessageDetailsAdapter.rowsAfterGap = rowsAfterGap
        notifyItemRangeInserted(indexToInsert, totalToInsert)
    }
}
`)
	for _, f := range findings {
		if f.Rule == "NoNameShadowing" {
			t.Fatalf("NoNameShadowing should ignore parameter assigned through labeled this, got: %s", f.Message)
		}
	}
}
...
func TestNaming_NoNameShadowing_SkipsScopeFunctionSelfConfiguration(t *testing.T) {
	findings := runRuleByName(t, "NoNameShadowing", `
package test
...
fun setUpPager(pager: Pager) {
    val pager = pager.apply {
        adapter = Adapter()
    }
    println(pager)
}
`)
	for _, f := range findings {
		if f.Rule == "NoNameShadowing" {
			t.Fatalf("NoNameShadowing should ignore same-name scope function self-configuration, got: %s", f.Message)
		}
	}
}

func TestNaming_NoNameShadowing_FlagsScopeFunctionDifferentReceiver(t *testing.T) {
	findings := runRuleByName(t, "NoNameShadowing", `
package test
...
fun setUpPager(pager: Pager, fallback: Pager) {
    val pager = fallback.apply {
        adapter = Adapter()
    }
    println(pager)
}
`)
	...
	if !found {
		t.Fatal("expected NoNameShadowing to flag scope function shadow using a different receiver")
	}
}

func TestNaming_NoNameShadowing_SkipsSafeCallLetSameNameLambdaParameter(t *testing.T) {
	findings := runRuleByName(t, "NoNameShadowing", `
package test

class Dialog

fun render(appDialog: Dialog?) {
    appDialog?.let { appDialog ->
        println(appDialog)
    }
}
`)
	for _, f := range findings {
		if f.Rule == "NoNameShadowing" {
			t.Fatalf("NoNameShadowing should ignore same-name safe-call let lambda parameter, got: %s", f.Message)
		}
	}
}

func TestNaming_NoNameShadowing_SkipsMutableDerivedCopy(t *testing.T) {
	findings := runRuleByName(t, "NoNameShadowing", `
package test

class Player(val durationMs: Long)

fun updateDuration(durationMs: Long, player: Player) {
    var durationMs = player.durationMs
    if (durationMs < 0) {
        durationMs = 0
    }
    println(durationMs)
...
`)
	for _, f := range findings {
		if f.Rule == "NoNameShadowing" {
			t.Fatalf("NoNameShadowing should ignore mutable derived copies, got: %s", f.Message)
		}
	}
}

func TestNaming_NoNameShadowing_SkipsMutableDerivedCopyFromDifferentPropertyName(t *testing.T) {
	findings := runRuleByName(t, "NoNameShadowing", `
package test

class Player(val duration: Long)
...
`)
	for _, f := range findings {
		if f.Rule == "NoNameShadowing" {
			t.Fatalf("NoNameShadowing should ignore mutable derived copies from differently named properties, got: %s", f.Message)
		}
	}
}

func TestNaming_NoNameShadowing_FlagsMutableShadowWithoutDerivedCopy(t *testing.T) {
	findings := runRuleByName(t, "NoNameShadowing", `
package test

fun updateDuration(durationMs: Long, fallbackMs: Long) {
    var durationMs = fallbackMs
    durationMs = 0
    println(durationMs)
}
`)
	...
	if !found {
		t.Fatal("expected NoNameShadowing to flag mutable shadows that are not derived from another object")
	}
}
...
func TestNaming_NoNameShadowing_AcceptsNullNarrowingIdiom(t *testing.T) {
	cases := []struct {
		...
		{"check_not_null", `val name = checkNotNull(name)`},
		{"require_not_null", `val name = requireNotNull(name)`},
		{"named_check_not_null", `val name = checkNotNull(value = name)`},
	}
	for _, tc := range cases {
...
func TestNaming_NoNameShadowing_FlagsCheckNotNullDifferentReceiver(t *testing.T) {
	findings := runRuleByName(t, "NoNameShadowing", `
package test

fun example(name: String?, fallback: String?) {
    val name = checkNotNull(fallback)
    println(name)
}
`)
	found := false
	...
	if !found {
		t.Fatal("expected NoNameShadowing to flag checkNotNull shadow using a different value")
	}
}
