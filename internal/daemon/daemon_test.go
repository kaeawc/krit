...
func TestIdleTimeoutClosesServer(t *testing.T) {
	...
	if err := Call(socket, "echo", nil, nil); err != nil {
		t.Fatalf("call: %v", err)
	}
	select {
	case <-time.After(2 * time.Second):
		t.Fatalf("server did not auto-stop after idle timeout")
	case <-waitClosed(srv):
	}
}
...
func TestIdleTimeoutDisabledByDefault(t *testing.T) {
	...
	if !Available(socket) {
		t.Fatalf("default daemon (IdleTimeout=0) should not auto-close")
	}
}
