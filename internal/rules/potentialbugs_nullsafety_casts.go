...
func unsafeCastHasNeverSucceedsDiagnosticFlat(ctx *api.Context, idx uint32) (oracle.Diagnostic, bool) {
	if ctx.File == nil || ctx.Resolver == nil {
...
func unsafeCastFindingMessage(cast unsafeCastExpressionParts, d oracle.Diagnostic) string {
	if d.Message != "" {
...
func unsafeCastKnownPlatformCastFlat(ctx *api.Context, source, target uint32, targetName string) bool {
	if ctx.File == nil {
		return false
	}
	file := ctx.File
...
func unsafeCastGetSystemServiceMatchesFlat(ctx *api.Context, call, target uint32, targetName string) bool {
	file := ctx.File
...
func unsafeCastPlatformCallConfirmedFlat(ctx *api.Context, call uint32, callee string, receiverOK func(*typeinfer.ResolvedType) bool) bool {
	file := ctx.File
...
func unsafeCastEnclosingOwnerMatchesFlat(ctx *api.Context, idx uint32, receiverOK func(*typeinfer.ResolvedType) bool) bool {
...
func unsafeCastOracleCallTargetFlat(ctx *api.Context, idx uint32) string {
	if ctx.Resolver == nil || ctx.File == nil {
		return ""
	}
...
func (r *UnsafeCastRule) check(ctx *api.Context) {
	...
	f := r.Finding(file, file.FlatRow(idx)+1, file.FlatCol(idx)+1,
		unsafeCastFindingMessage(cast, oracle.Diagnostic{}))
	f.Confidence = 0.95
	if !cast.safe {
		f.Fix = &scanner.Fix{
			ByteMode:    true,
...
func isCastGuardedByTypePredicateFlat(file *scanner.File, idx uint32, _ string, castTarget string) bool {
...
func (r *CastToNullableTypeRule) check(ctx *api.Context) {
	idx, file := ctx.Idx, ctx.File
	cast, ok := unsafeCastExpressionPartsFlat(file, idx)
	if !ok || cast.safe || file.FlatType(cast.target) != "nullable_type" {
		return
	}

	f := r.Finding(file, file.FlatRow(idx)+1, file.FlatCol(idx)+1,
		"Cast to nullable type. Use 'as?' for a safe cast instead of 'as Type?'.")
	typeName := strings.TrimSuffix(strings.TrimSpace(file.FlatNodeText(cast.target)), "?")
	f.Fix = &scanner.Fix{
		ByteMode:    true,
		StartByte:   int(file.FlatStartByte(idx)),
		EndByte:     int(file.FlatEndByte(idx)),
		Replacement: strings.TrimSpace(file.FlatNodeText(cast.source)) + " as? " + typeName,
	}
	ctx.Emit(f)
}
