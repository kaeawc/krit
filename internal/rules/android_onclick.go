...
func (r *OnClickRule) check(ctx *api.Context) {
	if ctx.ResourceIndex == nil || len(ctx.ParsedFiles) == 0 {
		return
	}
...
func emitOnClickFindings(ctx *api.Context, rule BaseRule, summary *onClickClassSummary, handlersByLayout map[string][]onClickHandlerSite) {
