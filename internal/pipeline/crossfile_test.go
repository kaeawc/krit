...
func TestCrossFilePhase_CrossFindingsCacheExcludesModuleAwareFindings(t *testing.T) {
	cacheDir := t.TempDir()
	codeIndex := scanner.BuildIndexFromData(nil, nil)
	codeIndex.Fingerprint = "test-fingerprint"

	crossRule := api.FakeRule("CrossRule", api.WithNeeds(api.NeedsCrossFile), api.WithCheck(func(ctx *api.Context) {
		ctx.Emit(scanner.Finding{File: "src/Foo.kt", Line: 1, Col: 1, Message: "cross"})
	}))
	moduleRule := api.FakeRule("ModuleRule", api.WithNeeds(api.NeedsModuleIndex), api.WithCheck(func(ctx *api.Context) {
		ctx.Emit(scanner.Finding{File: "src/Foo.kt", Line: 2, Col: 1, Message: "module"})
	}))

	graph := module.NewModuleGraph("/repo")
	graph.Modules[":app"] = &module.Module{Path: ":app", Dir: "/repo/app"}
	moduleIndex := &module.PerModuleIndex{Graph: graph}
	in := DispatchResult{
		IndexResult: IndexResult{
			ParseResult: ParseResult{
				ActiveRules: []*api.Rule{crossRule, moduleRule},
			},
			CodeIndex:             codeIndex,
			Graph:                 graph,
	...
	first, err := CrossFilePhase{}.Run(context.Background(), in)
	if err != nil {
		t.Fatal(err)
	}
	if got := first.Findings.Len(); got != 2 {
		t.Fatalf("first run findings = %d, want 2", got)
	}

	key := scanner.CrossFindingsKey(codeIndex.Fingerprint, in.RuleHash)
	cached, ok := scanner.LoadCrossFindings(cacheDir, key)
	if !ok {
		t.Fatal("cross findings cache did not save")
	}
	if got := cached.Len(); got != 1 {
		t.Fatalf("cached findings = %d, want only the cross-rule finding", got)
	}
	if got := cached.RuleAt(0); got != "CrossRule" {
		t.Fatalf("cached rule = %q, want CrossRule", got)
	}

	second, err := CrossFilePhase{}.Run(context.Background(), in)
	if err != nil {
		t.Fatal(err)
	}
	if got := second.Findings.Len(); got != 2 {
		t.Fatalf("second run findings = %d, want 2; got %+v", got, second.Findings.Findings())
	}
	counts := map[string]int{}
	for _, finding := range second.Findings.Findings() {
		counts[finding.Rule]++
	}
	if counts["CrossRule"] != 1 || counts["ModuleRule"] != 1 {
		t.Fatalf("second run rule counts = %+v, want one cross and one module finding", counts)
	}
}
