...
//
// Correctness bar is findings-equivalent, not byte-identical.
...
func FindRepoDir(scanPaths []string) string {
	return projectroot.Find(scanPaths)
}
...
// the file appears in N other files' closures. Pass nil for a
// fresh-hash-every-time slow path.
...
// cacheScopeCompatibleV2 checks cache entry scope compatibility with the call filter and declaration
...
// map across all VerifyClosure calls in the pass.
//
...
// AssembleOracle builds an Data from a set of cache hits plus any
...
func freshOracleEntryJobs(fresh *Data, deps *CacheDepsFile) []freshOracleEntryJob {
	if fresh == nil {
		return nil
	}
...
func cloneOracleClassMap(in map[string]*Class) map[string]*Class {
	if len(in) == 0 {
		return nil
	}
...
func newFreshEntryWriteStats(fresh *Data, deps *CacheDepsFile) *freshEntryWriteStats {
...
// per-entry closure computations so shared dependency closures are read once.
