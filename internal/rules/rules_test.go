...
func buildRuleIndex() map[string]*api.Rule {
	idx := make(map[string]*api.Rule, len(api.Registry))
...
func runRule(t *testing.T, rule *api.Rule, file *scanner.File) []scanner.Finding {
	t.Helper()
...
func TestPositiveFixtures(t *testing.T) {
	...
	err := filepath.Walk(positiveDir, func(path string, info os.FileInfo, err error) error {
		...
		if rule.Needs.Has(api.NeedsParsedFiles) {
			return nil
		}
		if rule.Needs.Has(api.NeedsCrossFile) {
			return nil
		}
		if rule.Needs.Has(api.NeedsModuleIndex) {
			return nil
		}
		if rule.Needs.Has(api.NeedsResources) {
			return nil
		}
...
func TestNonAndroidRulesHaveFixtures(t *testing.T) {
	...
	for _, r := range api.Registry {
		...
		if r.Needs.Has(api.NeedsCrossFile) || r.Needs.Has(api.NeedsModuleIndex) || r.Needs.Has(api.NeedsParsedFiles) {
			continue
		}
...
func TestDescriptionOfReturnsDescription(t *testing.T) {
	...
	// since Description is a field on api.Rule.
	for _, r := range api.Registry {
		if r.Description == "" {
