 ...
 * definition annotation. Go takes the first name of its scope list that
 * appears as `@<Name>` in the class's modifier text; with several scopes the
 * message names the one that comes first in that list, as in Go.
 *
 ...
 *   scoped, so none is reported here.
...
internal object ScopeOnParameterizedClass :
    ...
    // Scope annotations that carry no scope meta-annotation.
    ...
    override fun check(declaration: FirRegularClass) {
        ...
        report(
            ...
            "@$scope on generic class '${declaration.name.asString()}' shares one instance across all type " +
