...
// TestConfigAdapter_AliasFallback exercises api.ApplyConfig's primary-
...
func TestConfigAdapter_AliasFallback(t *testing.T) {
	...
	// Build a minimal descriptor so we can verify api.ApplyConfig's
	// probe order independently.
	desc := api.RuleDescriptor{
		ID:      "LongMethod",
		RuleSet: "complexity",
	...
	api.ApplyConfig(rule, desc, adapter)
	...
	api.ApplyConfig(rule2, desc, NewConfigAdapter(cfg2))
