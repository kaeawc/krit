...
func (ps *parsePackStore) SaveEncodedEntries(writes []parseEncodedEntryWrite) (int, error) {
	...
	for i := 0; i < workers; i++ {
		...
		go func() {
			defer wg.Done()
			for job := range jobs {
				changed, err := job.handle.putMany(job.writes)
				if err != nil {
					select {
					case errCh <- err:
					default:
...
func (h *parsePackHandle) putMany(writes []parseEncodedEntryWrite) (bool, error) {
	if len(writes) == 0 {
		return false, nil
	}
	...
	latest := make(map[string]parsePackItem, len(writes))
	for _, w := range writes {
		if w.hash == "" || w.data == nil {
			continue
		}
		latest[w.hash] = parsePackItem{key: w.hash, data: w.data, crc: crc32.ChecksumIEEE(w.data)}
	}
	if len(latest) == 0 {
		return false, nil
	}
	for key, item := range latest {
		e, ok := existingIndex[key]
		if !ok || e.length != uint64(len(item.data)) || e.crc != item.crc {
			continue
		}
		delete(latest, key)
	}
...
// mustParsePackHeader re-parses a buffer that buildParsePack just produced
// in-memory. A failure here means buildParsePack and parseParsePackHeader
// have drifted — a programmer error, not an IO condition. Disk-load paths
// use parseParsePackHeader directly and handle errors normally.
