...
func registerStyleBracesRules() {
	...
	// --- from style_braces.go ---
	{
		...
		v2.Register(&v2.Rule{
			...
			NodeTypes: []string{"if_expression"}, Confidence: 0.75, Fix: v2.FixCosmetic, Implementation: r,
			Check: r.check,
		})
	}
	{
		...
		v2.Register(&v2.Rule{
			ID: r.RuleName, Category: r.RuleSetName, Description: r.Desc, Sev: v2.Severity(r.Sev),
			NodeTypes: []string{"when_entry"}, Confidence: 0.75, Fix: v2.FixCosmetic, Implementation: r,
			Check: r.check,
		})
	}
	{
		...
		v2.Register(&v2.Rule{
			ID: r.RuleName, Category: r.RuleSetName, Description: r.Desc, Sev: v2.Severity(r.Sev),
			NodeTypes: []string{"for_statement", "while_statement", "do_while_statement"}, Confidence: 0.75, Fix: v2.FixCosmetic, Implementation: r,
			Check: r.check,
		})
	}
}
