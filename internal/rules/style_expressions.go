...
func getJumpMetricsFlat(idx uint32, file *scanner.File) jumpMetrics {
	...
	return filefacts.NodeFact(fileFactsCache(), file, idx, slotJumpMetrics, func() jumpMetrics {
...
// Entries with empty FQN lists are accepted on identifier shape alone and must
...
// `<package-of-fqn>.*`. Results are memoized via the shared filefacts
// cache (per-run, per-file).
func fileImportsFQN(file *scanner.File, fqn string) bool {
	if file == nil || fqn == "" {
		return false
	}
	return fileFactsCache().Imports(file).HasFQN(fqn)
}
...
func flatSubtreeHasNodeType(file *scanner.File, idx uint32, nodeType string) bool {
	...
	file.FlatWalkAllNodes(idx, func(candidate uint32) {
		...
		if file.FlatType(candidate) == nodeType {
			found = true
		}
	})
	return found
...
func (r *VarCouldBeValRule) reassignedNamesFlat(scope uint32, file *scanner.File) map[string]bool {
	return filefacts.NodeFact(fileFactsCache(), file, scope, slotScopeReassignments, func() map[string]bool {
...
func prefixConstantExpressionFlat(ctx *api.Context, expr uint32) bool {
	file := ctx.File
	seenSign := false
...
func binaryConstantExpressionFlat(ctx *api.Context, expr uint32) bool {
	file := ctx.File
...
func isNumericLiteral(s string) bool {
	...
	if numericHasOperatorChar(s) {
		return false
	}
	return numericDigitsOnly(s)
}

func numericHasOperatorChar(s string) bool {
	for _, c := range s {
		switch c {
		case ' ', '\t', '(', ')', '+', '-', '*', '/', '%',
			'&', '|', '^', '!', '~', '<', '>', '=':
...
func numericDigitsOnly(s string) bool {
	sawDot := false
	for i := 0; i < len(s); i++ {
		c := s[i]
		if c >= '0' && c <= '9' {
