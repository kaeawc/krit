...
func (pc *XMLParseCache) Close() error {
	...
	if pc.lru != nil {
		if removed, err := pc.lru.Evict(); err != nil {
			errs = append(errs, err)
		} else if removed > 0 {
			pc.evictions.Add(int64(removed))
		}
...
// HasWrites reports whether this process queued or completed XML parse-cache
// writes. Read-hit LRU touches do not count.
func (pc *XMLParseCache) HasWrites() bool {
	if pc == nil {
		return false
	}
	stats := pc.writer.Stats()
	return stats.Queued > 0 || stats.Completed > 0 || pc.lastWriteSec.Load() > 0
}
