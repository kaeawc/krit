...
func ScanProfile(ctx context.Context, opts ScanOptions, profile string) (*ScanResult, error) {
	...
	if len(out) == 0 {
		return nil, fmt.Errorf("krit produced no output for profile %q: %w", profile, runErr)
	}
...
func ScanProfileWithProgress(
	...
	if len(out) == 0 {
		...
		return nil, fmt.Errorf("krit produced no output for profile %q: %w", profile, waitErr)
	}
