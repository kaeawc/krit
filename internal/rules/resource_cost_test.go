...
func TestDatabaseQueryOnMainThread_PositiveJavaDatabaseServiceMessage(t *testing.T) {
...
class DatabaseTable {
  AppSQLiteDatabase readableDatabase;
}
...
class SmsSendtoActivity extends android.app.Activity {
  ...
  private android.content.Intent getNextIntent(android.content.Intent original) {
    AppDatabase.threads().getOrCreateThreadIdFor(new Object());
    return original;
  }
}
`)
	findings := runDatabaseQueryOnMainThreadFiles(t, file)
	if len(findings) != 1 {
		t.Fatalf("expected one database service helper finding, got %d", len(findings))
	}
	message := findings[0].Message
	if !strings.Contains(message, "getNextIntent()") ||
		!strings.Contains(message, "AppDatabase.threads().getOrCreateThreadIdFor") ||
		...
		t.Fatalf("expected explicit database service call and fix guidance, got %q", message)
	}
}
...
func TestDatabaseQueryOnMainThread_NegativeJavaSimpleTaskAppDatabase(t *testing.T) {
	file := parseJavaInline(t, `
package test;
...
class MediaPreviewFragment extends android.app.Fragment {
  public void onResume() {
    SimpleTask.run(() -> AppDatabase.messages().getMessageRecord(1), result -> {});
  }
}
`)
	findings := runDatabaseQueryOnMainThreadFiles(t, file)
	if len(findings) != 0 {
		t.Fatalf("expected no findings for AppDatabase work inside Java SimpleTask.run, got %d", len(findings))
	}
}

func TestDatabaseQueryOnMainThread_NegativeJavaRxSubscribeOnAppDatabase(t *testing.T) {
	file := parseJavaInline(t, `
package test;
...
class ShowAdminsBottomSheetDialog extends android.app.Fragment {
  ...
  private static Cursor getAdmins(long groupId) {
    return AppDatabase.groups().getGroup(groupId);
  }
}
`)
	findings := runDatabaseQueryOnMainThreadFiles(t, file)
	if len(findings) != 0 {
		t.Fatalf("expected no findings for AppDatabase work inside Java Rx subscribeOn(IO), got %d", len(findings))
	}
}
...
open class DatabaseTable {
  val readableDatabase: AppSQLiteDatabase = TODO()
}
class AppSQLiteDatabase {
...
class MediaFragment : android.app.Fragment() {
  ...
  private fun deleteMedia() {
    AppDatabase.messages.getMessageRecord(1)
  }
}
`)
	if len(findings) != 1 {
