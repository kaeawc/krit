...
var compileSDKValueRe = regexp.MustCompile(`^\s*compileSdk(?:Version)?\s*[=(]?\s*(\d+)`)
...
func (r *CompileSdkMismatchAcrossModulesRule) check(ctx *api.Context) {
	pmi := ctx.ModuleIndex
...
func collectAndroidModuleCompileSDKs(graph *module.Graph) []moduleCompileSDK {
