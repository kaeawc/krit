...
// kindName returns a human-readable label for a Kind.
...
// broken down by Kind (the top-level directory name encodes the kind).
func (s *FileStore) Stats() (Stats, error) {
	st := Stats{ByKind: make(map[Kind]KindStats)}
