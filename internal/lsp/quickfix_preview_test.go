...
func TestFindingsToCodeActionsAddsPreviewForIdiomaticFix(t *testing.T) {
	...
	if !strings.Contains(actions[0].Description, "```diff") {
		t.Fatalf("expected markdown diff description, got %q", actions[0].Description)
	}
...
func TestFindingsToCodeActionsSkipsPreviewForCosmeticFix(t *testing.T) {
	...
	if actions[0].Description != "" {
		t.Fatalf("expected no preview description for cosmetic fix, got %q", actions[0].Description)
	}
}
