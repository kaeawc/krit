...
func TestSizeCapLRU_EvictUsesBatchRemove(t *testing.T) {
	l, _ := newLRU(t, 300)
	h1 := hash64(1)
	h2 := hash64(2)
	h3 := hash64(3)
	...
	var batches [][]string
	l.RemoveBatch = func(hashes []string) error {
	...
	if len(batches) != 1 {
		t.Fatalf("expected one batch removal, got %d", len(batches))
	}
	if got := batches[0]; len(got) != 2 || got[0] != h1 || got[1] != h2 {
		t.Fatalf("unexpected batch hashes: %v", got)
	}
	stats := l.Stats()
	if stats.Entries != 2 || stats.Bytes != 200 {
		t.Fatalf("unexpected post-evict stats: entries=%d bytes=%d", stats.Entries, stats.Bytes)
	}
}
