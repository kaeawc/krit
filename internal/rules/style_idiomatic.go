...
func flatNullOrEmptyEmptinessCheck(ctx *api.Context, node uint32) flatNullOrEmptyCheck {
	file := ctx.File
	node = flatUnwrapParenExpr(file, node)
...
func flatNullOrEmptyCallCheck(ctx *api.Context, call uint32) flatNullOrEmptyCheck {
	file := ctx.File
...
func flatNullOrEmptyEqualityCheck(ctx *api.Context, node uint32) flatNullOrEmptyCheck {
	file := ctx.File
	left, op, right := flatEqualityExpressionParts(file, node)
	if left == 0 || right == 0 || op != "==" {
		return flatNullOrEmptyCheck{}
...
func flatNullOrEmptySizeLikeCheck(ctx *api.Context, node uint32) flatNullOrEmptyCheck {
	file := ctx.File
	node = flatUnwrapParenExpr(file, node)
	switch file.FlatType(node) {
	case "call_expression":
...
func flatResolvedEmptyCallTargetEvidence(ctx *api.Context, call uint32, name string) (semantics.SemanticEvidence, bool) {
...
func flatNullOrEmptyReceiverSupported(ctx *api.Context, receiver uint32, kind nullOrEmptyCheckKind) (semantics.SemanticEvidence, bool) {
...
func flatNullOrEmptyConfidence(ctx *api.Context, evidence semantics.SemanticEvidence) (float64, bool) {
	base := 0.75
	if ctx != nil {
...
func flatThrowPattern(ctx *api.Context, nodeType, nodeText string, exceptionType, replacement string, base BaseRule) {
	...
	condNode, bodyNode := flatThrowExtractCondAndBody(file, idx)
	if condNode == 0 || bodyNode == 0 {
		return
	}
	innerCondNode := flatThrowUnwrapNegatedCond(file, condNode)
	if innerCondNode == 0 {
		return
	}
	throwNode := flatThrowLocateThrowNode(file, bodyNode)
	if throwNode == 0 {
		return
	}
	callNode := flatThrowFindCallNode(file, throwNode)
	if callNode == 0 {
		return
	}
...
func flatThrowExtractCondAndBody(file *scanner.File, idx uint32) (condNode, bodyNode uint32) {
	...
	return condNode, bodyNode
}

func flatThrowUnwrapNegatedCond(file *scanner.File, condNode uint32) uint32 {
	...
	return innerCondNode
}

func flatThrowIsThrowJump(file *scanner.File, node uint32) bool {
...
func flatThrowLocateThrowNode(file *scanner.File, bodyNode uint32) uint32 {
	actualBody := bodyNode
	...
	case "jump_expression":
		if flatThrowIsThrowJump(file, actualBody) {
			return actualBody
		}
	case "block":
		if file.FlatNamedChildCount(actualBody) == 1 {
			child := file.FlatNamedChild(actualBody, 0)
			if flatThrowIsThrowJump(file, child) {
				return child
			}
		}
	}
...
func flatThrowFindCallNode(file *scanner.File, throwNode uint32) uint32 {
