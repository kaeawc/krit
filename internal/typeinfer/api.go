...
type defaultResolver struct {
	...
	// javaTopLevelNames lazily memoizes the set of top-level type
	// names declared in a Java file, keyed by file path. Populated on
	// first ResolveImport that needs it. Java files are not indexed by
...
func (r *defaultResolver) ResolveImport(simpleName string, file *scanner.File) string {
	// A class/interface/object declared in the same file shadows any
	// implicit java.lang.* auto-import for that simple name. Check the
	// indexed classes first so a local `class Runtime` lookalike does
	// not get resolved to `java.lang.Runtime`.
	if file != nil {
		if info, ok := r.classes[simpleName]; ok && info != nil && info.File == file.Path {
			if info.FQN != "" {
				return info.FQN
			}
			return info.Name
	...
	if fqn, ok := KotlinAutoImports[simpleName]; ok {
		...
		// shadows the implicit java.lang.* import. The Java indexer
		// does not populate r.classes today, so look directly at the
		// file's tree-sitter declarations.
		if file != nil && file.Language == scanner.LangJava && r.javaFileDeclaresTypeName(file, simpleName) {
			return ""
		}
		return fqn
	}
...
// javaFileDeclaresTypeName reports whether a Java file declares a
// top-level class, interface, enum, or record with the given simple
// name. Used to reject implicit java.lang.* auto-imports when the
// same-file declaration shadows the JDK type. The set of declared
// names is memoized per-file via javaTopLevelNames.
func (r *defaultResolver) javaFileDeclaresTypeName(file *scanner.File, simpleName string) bool {
	if file == nil || file.FlatTree == nil || simpleName == "" {
		return false
	}
	if cached, ok := r.javaTopLevelNames.Load(file.Path); ok {
		_, found := cached.(map[string]struct{})[simpleName]
		return found
	}
	...
	for child := file.FlatFirstChild(0); child != 0; child = file.FlatNextSib(child) {
		...
		case "class_declaration", "interface_declaration", "enum_declaration", "record_declaration":
			for sub := file.FlatFirstChild(child); sub != 0; sub = file.FlatNextSib(sub) {
				if file.FlatType(sub) == "identifier" {
					names[file.FlatNodeText(sub)] = struct{}{}
					break
				}
			}
		}
	}
	r.javaTopLevelNames.Store(file.Path, names)
	_, found := names[simpleName]
	return found
}
...
func (r *defaultResolver) inferExpressionTypeFlat(idx uint32, file *scanner.File, it *ImportTable) *ResolvedType {
	...
	if t := r.inferLiteralTypeFlat(idx, file, it); t != nil {
		return t
	}
	return r.inferCompositeExprTypeFlat(idx, file, it)
}

func (r *defaultResolver) inferLiteralTypeFlat(idx uint32, file *scanner.File, it *ImportTable) *ResolvedType {
	switch file.FlatType(idx) {
	case "string_literal", "line_string_literal", "multi_line_string_literal":
		return r.makeResolvedType("String", it, false)
	case "integer_literal":
	...
	case "long_literal":
		return r.makeResolvedType("Long", it, false)
	case "real_literal":
		return r.inferRealLiteralTypeFlat(idx, file, it)
	case "boolean_literal":
		return r.makeResolvedType("Boolean", it, false)
	case "character_literal":
		return r.makeResolvedType("Char", it, false)
...
func (r *defaultResolver) inferCompositeExprTypeFlat(idx uint32, file *scanner.File, it *ImportTable) *ResolvedType {
	switch file.FlatType(idx) {
	case "call_expression":
		return r.inferCallExpressionTypeFlat(idx, file, it)
	case "this_expression":
		return r.inferThisExprTypeFlat(idx, file)
	case "simple_identifier":
		return r.inferSimpleIdentTypeFlat(idx, file)
	case "navigation_expression":
		return r.inferNavigationExpressionTypeFlat(idx, file, it)
	case "parenthesized_expression":
		return r.inferParenExprTypeFlat(idx, file, it)
	case "additive_expression":
		return r.inferAdditiveExprTypeFlat(idx, file, it)
	case "multiplicative_expression":
		return r.inferMultiplicativeExprTypeFlat(idx, file, it)
	case "comparison_expression", "equality_expression", "conjunction_expression", "disjunction_expression":
		return r.makeResolvedType("Boolean", it, false)
	case "prefix_expression":
		return r.inferPrefixExprTypeFlat(idx, file, it)
	...
	case "as_expression":
		return r.inferAsExprTypeFlat(idx, file, it)
	}
	return UnknownType()
}

func (r *defaultResolver) inferRealLiteralTypeFlat(idx uint32, file *scanner.File, it *ImportTable) *ResolvedType {
	text := file.FlatNodeText(idx)
	if strings.HasSuffix(strings.ToLower(text), "f") {
		return r.makeResolvedType("Float", it, false)
	}
...
func (r *defaultResolver) inferThisExprTypeFlat(idx uint32, file *scanner.File) *ResolvedType {
...
func (r *defaultResolver) inferSimpleIdentTypeFlat(idx uint32, file *scanner.File) *ResolvedType {
...
func (r *defaultResolver) inferParenExprTypeFlat(idx uint32, file *scanner.File, it *ImportTable) *ResolvedType {
	for i := 0; i < file.FlatChildCount(idx); i++ {
		child := file.FlatChild(idx, i)
		switch file.FlatType(child) {
		case "(", ")":
...
func (r *defaultResolver) inferAdditiveExprTypeFlat(idx uint32, file *scanner.File, it *ImportTable) *ResolvedType {
...
func (r *defaultResolver) inferMultiplicativeExprTypeFlat(idx uint32, file *scanner.File, it *ImportTable) *ResolvedType {
	left := r.inferExpressionTypeFlat(file.FlatChild(idx, 0), file, it)
...
func (r *defaultResolver) inferPrefixExprTypeFlat(idx uint32, file *scanner.File, it *ImportTable) *ResolvedType {
...
func (r *defaultResolver) inferAsExprTypeFlat(idx uint32, file *scanner.File, it *ImportTable) *ResolvedType {
	foundAs := false
	isSafe := false
	for i := 0; i < file.FlatChildCount(idx); i++ {
		child := file.FlatChild(idx, i)
...
func (r *defaultResolver) inferCallExpressionTypeFlat(idx uint32, file *scanner.File, it *ImportTable) *ResolvedType {
	...
	if file.FlatType(firstChild) == "navigation_expression" && funcName != "" {
		if t := r.inferCallOnNavigationExprFlat(idx, firstChild, funcName, file, it); t != nil {
			return t
		}
	}

	return r.inferCallByFuncNameFlat(funcName, it)
}

func (r *defaultResolver) inferCallOnNavigationExprFlat(_ uint32, navIdx uint32, funcName string, file *scanner.File, _ *ImportTable) *ResolvedType {
	receiverIdx := flatNavigationReceiver(file, navIdx)
