...
import (
	...
	"image"
	"image/gif"
	_ "image/jpeg" // import for side effects: register JPEG decoder
	_ "image/png"  // import for side effects: register PNG decoder
	"io"
	"os"
	"path/filepath"
	"runtime"
...
func ScanIconDirsWithWorkers(resDir string, maxWorkers int) (*IconIndex, error) {
	...
	for i, input := range inputs {
		...
		g.Go(func() error {
			...
			if err != nil {
				return nil //nolint:nilerr // skip-and-continue: density-dir unreadable, other dirs proceed
			}
...
func detectFormatFromContent(data []byte) string {
	...
	if f := detectByMagicBytes(data); f != "" {
		return f
	}
	return detectXMLContent(data)
}
...
func detectXMLContent(data []byte) string {
