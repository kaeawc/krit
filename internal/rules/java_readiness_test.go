...
type javaReadiness struct {
	Status   api.LanguageSupportStatus `yaml:"status"`
...
func TestMetaForRuleIncludesJavaSupportClassification(t *testing.T) {
	explicit := mustV2RuleByID(t, "AddJavascriptInterface")
	meta, ok := MetaForRule(explicit)
	if !ok {
	...
	if support.Status != api.LanguageSupportPending {
		t.Fatalf("LongMethod Java support = %s, want %s", support.Status, api.LanguageSupportPending)
	}
	if support.Reason == "" {
		t.Fatal("LongMethod Java support should explain why Java support is pending")
	}
}
...
func assertReadinessMapsMirrorCode(t *testing.T, label string, doc map[string]javaReadiness, code map[string]api.LanguageSupport) {
	t.Helper()
...
func (entry javaReadiness) toLanguageSupport() api.LanguageSupport {
...
func canonicalLanguageSupport(entry api.LanguageSupport) api.LanguageSupport {
...
func mustV2RuleByID(t *testing.T, id string) *api.Rule {
	t.Helper()
	for _, r := range api.Registry {
		if r != nil && r.ID == id {
			return r
