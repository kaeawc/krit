...
func collectMessages(data []byte) ([]json.RawMessage, error) {
	...
	return msgs, nil //nolint:nilerr // test helper: read-loop breaks on EOF/short-read and returns whatever messages it gathered
}
...
// TestWorkspaceCacheReusedAcrossRequests verifies that the LSP server's
// embedded WorkspaceState memoizes parsed files, so consecutive analyses
// of the same buffer don't re-run tree-sitter.
func TestWorkspaceCacheReusedAcrossRequests(t *testing.T) {
	...
	initializedNotif := buildRequest(nil, "initialized", map[string]interface{}{})
	didOpen := buildRequest(nil, "textDocument/didOpen", map[string]interface{}{
		"textDocument": map[string]interface{}{
			"uri":        "file:///tmp/test/Same.kt",
			"languageId": "kotlin",
			"version":    1,
			"text":       source,
		},
	...
	// didOpen triggers analyzeAndPublish (via ParseFile) and
	// publishDiagnostics may also call getDocumentFlatFile during
	// the same pass — both should fold into one ParseFile cache miss
	// followed by zero or more hits, never two misses on the same
	// content.
	stats := server.workspace.Stats()
	if stats.Misses != 1 {
		t.Errorf("workspace misses after didOpen: got %d, want 1", stats.Misses)
	}
	if stats.ParsedEntries != 1 {
		t.Errorf("workspace entries: got %d, want 1", stats.ParsedEntries)
	}
}

// TestWorkspaceCacheInvalidatedOnDidClose verifies that closing a
// buffer drops its cached parse entry from the workspace.
func TestWorkspaceCacheInvalidatedOnDidClose(t *testing.T) {
	source := "package com.example\n\nfun foo() = 42\n"
	initReq := buildRequest(1, "initialize", map[string]interface{}{
		"processId":    1234,
		"rootUri":      "file:///tmp/test",
	...
	didOpen := buildRequest(nil, "textDocument/didOpen", map[string]interface{}{
		"textDocument": map[string]interface{}{
			"uri":        "file:///tmp/test/Bye.kt",
			"languageId": "kotlin",
			"version":    1,
			"text":       source,
		},
	})
	didClose := buildRequest(nil, "textDocument/didClose", map[string]interface{}{
		"textDocument": map[string]interface{}{"uri": "file:///tmp/test/Bye.kt"},
	})
	...
	input.Write(initializedNotif)
	input.Write(didOpen)
	input.Write(didClose)

	var output bytes.Buffer
	server := NewServer(bufio.NewReader(&input), &output)
	server.Run()

	if got := server.workspace.Stats().ParsedEntries; got != 0 {
		t.Errorf("workspace entries after didClose: got %d, want 0", got)
	}
}
...
func TestOracleRefreshDebouncedSeparately(t *testing.T) {
	oldDelay := debounceDelay
	debounceDelay = 10 * time.Millisecond
	defer func() { debounceDelay = oldDelay }()
	oldOracleDelay := oracleDebounceDelay
	oracleDebounceDelay = 60 * time.Millisecond
	defer func() { oracleDebounceDelay = oldOracleDelay }()

	uri := "file:///tmp/test/Oracle.kt"
	initReq := buildRequest(1, "initialize", map[string]interface{}{
		"processId":    1234,
		"rootUri":      "file:///tmp/test",
		"capabilities": map[string]interface{}{},
	...
	mkChange := func(version int, body string) []byte {
		return buildRequest(nil, "textDocument/didChange", map[string]interface{}{
			"textDocument":   map[string]interface{}{"uri": uri, "version": version},
			"contentChanges": []map[string]interface{}{{"text": body}},
		})
	}
	...
	input.Write(initializedNotif)
	input.Write(didOpenReq)
	input.Write(mkChange(2, "package com.example\nval x = 1\n"))
	input.Write(mkChange(3, "package com.example\nval x = 2\n"))
	input.Write(mkChange(4, "package com.example\nval x = 3\n"))
	...
	server.OracleRefresh = func(_ string, content []byte) {
		oracleMu.Lock()
		defer oracleMu.Unlock()
		oracleCalls++
		lastContent = append([]byte(nil), content...)
	}

	server.Run()

	// Wait for the oracle timer to fire (longer than diagnostic timer).
	...
	defer oracleMu.Unlock()
	if oracleCalls != 1 {
		t.Errorf("expected oracle refresh to coalesce 3 changes into 1 call, got %d", oracleCalls)
	}
	if string(lastContent) != "package com.example\nval x = 3\n" {
		t.Errorf("oracle refresh saw stale content: %q", string(lastContent))
	}
}
...
func TestHoverReturnsRuleInfo(t *testing.T) {
	...
	uri := "file:///tmp/test/Test.kt"
	content := []byte("val x = 42\n")
	start := strings.Index(string(content), "42")
	server.docs[uri] = &Document{
		URI:     uri,
		Content: content,
		Findings: scanner.CollectFindings([]scanner.Finding{
			{
				File: "/tmp/test/Test.kt", Line: 1, Col: start, StartByte: start, EndByte: start + len("42"),
	...
	params := HoverParams{
		TextDocument: TextDocumentIdentifier{URI: uri},
		Position:     Position{Line: 0, Character: uint32(start)}, // line 0 = finding line 1
	}
	paramsBytes, _ := json.Marshal(params)
	req := &Request{
		JSONRPC: "2.0",
	...
	if !strings.Contains(hover.Contents.Value, "default: warning") {
		t.Errorf("expected hover to contain default severity, got: %s", hover.Contents.Value)
	}
	if !strings.Contains(hover.Contents.Value, "current: warning") {
		t.Errorf("expected hover to contain current severity, got: %s", hover.Contents.Value)
	}
...
func TestHoverOnlyMatchesFindingByteRange(t *testing.T) {
	server := &Server{
		docs: make(map[string]*Document),
	}

	uri := "file:///tmp/test/Test.kt"
	content := []byte("val a = 42; val b = 100\n")
	startA := strings.Index(string(content), "42")
	startB := strings.Index(string(content), "100")
	server.docs[uri] = &Document{
		URI:     uri,
		Content: content,
		Findings: scanner.CollectFindings([]scanner.Finding{
			{
				File: "/tmp/test/Test.kt", Line: 1, Col: startA, StartByte: startA, EndByte: startA + len("42"),
				RuleSet: "style", Rule: "MagicNumber", Severity: "warning",
				Message: "first literal",
			},
			{
				File: "/tmp/test/Test.kt", Line: 1, Col: startB, StartByte: startB, EndByte: startB + len("100"),
				RuleSet: "style", Rule: "MagicNumber", Severity: "warning",
				Message: "second literal",
			},
		}),
	}
	...
	params := HoverParams{
		TextDocument: TextDocumentIdentifier{URI: uri},
		Position:     Position{Line: 0, Character: uint32(startB + 1)},
	}
	paramsBytes, _ := json.Marshal(params)
	...
	if strings.Contains(hover.Contents.Value, "first literal") {
		t.Errorf("hover should not include first finding outside the hovered range, got: %s", hover.Contents.Value)
	}
	if !strings.Contains(hover.Contents.Value, "second literal") {
		t.Errorf("hover should include hovered finding, got: %s", hover.Contents.Value)
	}
}
...
func TestCodeActionReturnsGoToTestCommand(t *testing.T) {
	root := t.TempDir()
	mainPath := filepath.Join(root, "src", "main", "kotlin", "com", "example", "UserRepository.kt")
	testPath := filepath.Join(root, "src", "test", "kotlin", "com", "example", "UserRepositoryTest.kt")
	mainContent := []byte(`package com.example

class UserRepository {
    fun get(): String = "ok"
}
`)
	testContent := []byte(`package com.example
...
`)
	...
	server := &Server{
		...
		docs: map[string]*Document{
			uri: {URI: uri, Content: mainContent},
		},
	}
	...
	paramsBytes, _ := json.Marshal(params)
	req := &Request{JSONRPC: "2.0", ID: float64(1), Method: "textDocument/codeAction", Params: paramsBytes}

	var output bytes.Buffer
	server.writer = &output
	server.handleCodeAction(req)
	...
	if len(actions) != 1 {
		t.Fatalf("expected 1 go-to-test action, got %d: %#v", len(actions), actions)
	}
	if actions[0].Title != "Go to test: UserRepositoryTest.kt" {
		t.Fatalf("title: got %q", actions[0].Title)
	}
	if actions[0].Command == nil || actions[0].Command.Command != "krit.goToTest" {
		t.Fatalf("expected krit.goToTest command, got %#v", actions[0].Command)
	}
	argBytes, _ := json.Marshal(actions[0].Command.Arguments[0])
	var loc Location
	if err := json.Unmarshal(argBytes, &loc); err != nil {
		t.Fatal(err)
	}
	if loc.URI != pathToURI(testPath) {
		t.Fatalf("location URI: got %q, want %q", loc.URI, pathToURI(testPath))
	}
	if loc.Range.Start.Line != 2 {
		t.Fatalf("location line: got %d, want 2", loc.Range.Start.Line)
	}

	output.Reset()
	execParams := map[string]interface{}{"command": "krit.goToTest", "arguments": []interface{}{loc}}
	execBytes, _ := json.Marshal(execParams)
	server.handleExecuteCommand(&Request{JSONRPC: "2.0", ID: float64(2), Method: "workspace/executeCommand", Params: execBytes})
	msgs, _ = collectMessages(output.Bytes())
	json.Unmarshal(msgs[0], &resp)
	resultBytes, _ = json.Marshal(resp.Result)
	var returned Location
	json.Unmarshal(resultBytes, &returned)
	if returned.URI != loc.URI || returned.Range.Start != loc.Range.Start {
		t.Fatalf("executeCommand returned %#v, want %#v", returned, loc)
	}
}

func TestCodeActionOmitsGoToTestWithoutTestReference(t *testing.T) {
	root := t.TempDir()
	mainPath := filepath.Join(root, "src", "main", "kotlin", "com", "example", "UserRepository.kt")
	mainContent := []byte(`package com.example
...
`)
	...
	for _, action := range actions {
		if action.Command != nil && action.Command.Command == "krit.goToTest" {
			t.Fatalf("go-to-test action should be omitted without test references: %#v", action)
		}
	}
}
...
func TestCodeLensReturnsFunctionMetrics(t *testing.T) {
	server := &Server{
		docs: make(map[string]*Document),
	}
	uri := "file:///tmp/test/Test.kt"
...
`)
	...
	if len(lenses) != 2 {
		t.Fatalf("expected 2 code lenses, got %d: %+v", len(lenses), lenses)
	}
	if lenses[0].Command == nil || !strings.Contains(lenses[0].Command.Title, "complexity=") {
		t.Fatalf("expected metric command title, got %+v", lenses[0].Command)
	}
	if lenses[1].Command == nil || !strings.Contains(lenses[1].Command.Title, "consumers") {
		t.Fatalf("expected consumer count title, got %+v", lenses[1].Command)
	}
}
