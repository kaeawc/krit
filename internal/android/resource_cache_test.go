...
const sampleLayoutXML = `<?xml version="1.0" encoding="utf-8"?>
...
const changedLayoutXML = `<?xml version="1.0" encoding="utf-8"?>
<LinearLayout xmlns:android="http://schemas.android.com/apk/res/android"
    android:orientation="vertical"
    android:layout_width="match_parent"
    android:layout_height="match_parent">
    <TextView android:id="@+id/changed_title"
        android:layout_width="wrap_content"
        android:layout_height="wrap_content" />
</LinearLayout>
`

const sampleDrawableSelectorXML = `<?xml version="1.0" encoding="utf-8"?>
...
const changedDrawableSelectorXML = `<?xml version="1.0" encoding="utf-8"?>
<selector xmlns:android="http://schemas.android.com/apk/res/android">
...
func writeSampleLayoutDir(t *testing.T) string {
	t.Helper()
	dir := t.TempDir()
	...
	if err := os.WriteFile(filepath.Join(layoutDir, "main.xml"), []byte(sampleLayoutXML), 0o644); err != nil {
		t.Fatalf("write main.xml: %v", err)
	}
	return layoutDir
}

func writeSampleDrawableDir(t *testing.T) string {
	t.Helper()
	dir := t.TempDir()
	drawableDir := filepath.Join(dir, "drawable")
	if err := os.MkdirAll(drawableDir, 0o755); err != nil {
		t.Fatalf("mkdir drawable: %v", err)
	}
	if err := os.WriteFile(filepath.Join(drawableDir, "button.xml"), []byte(sampleDrawableSelectorXML), 0o644); err != nil {
		t.Fatalf("write button.xml: %v", err)
	}
	if err := os.WriteFile(filepath.Join(drawableDir, "logo.png"), []byte("png-bytes"), 0o644); err != nil {
...
func TestResourceIndexCache_LayoutRoundTripFindingEquivalence(t *testing.T) {
	layoutDir := writeSampleLayoutDir(t)

	prev := ActiveResourceIndexCache()
	SetActiveResourceIndexCache(nil)
	cold := newResourceIndex()
	if err := cold.scanLayoutDir(layoutDir, runtime.NumCPU()); err != nil {
		t.Fatalf("cold scan: %v", err)
	}
	SetActiveResourceIndexCache(prev)
	...
	warm1 := newResourceIndex()
	if err := warm1.scanLayoutDir(layoutDir, runtime.NumCPU()); err != nil {
		t.Fatalf("prime scan: %v", err)
	}
	if err := cache.Close(); err != nil {
		t.Fatalf("Close after prime scan: %v", err)
	...
	warm2 := newResourceIndex()
	if err := warm2.scanLayoutDir(layoutDir, runtime.NumCPU()); err != nil {
		t.Fatalf("warm scan: %v", err)
	}
	if s := cache.Stats(); s.Hits != 1 || s.Misses != 1 {
		t.Errorf("after warm hit: hits=%d misses=%d (want 1/1)", s.Hits, s.Misses)
	...
	if !reflect.DeepEqual(cold.Layouts, warm2.Layouts) {
		t.Errorf("Layouts drift cold vs warm:\n cold=%#v\n warm=%#v", cold.Layouts, warm2.Layouts)
	}
	if !reflect.DeepEqual(cold.LayoutConfigs, warm2.LayoutConfigs) {
		t.Errorf("LayoutConfigs drift cold vs warm:\n cold=%#v\n warm=%#v", cold.LayoutConfigs, warm2.LayoutConfigs)
	}
	if !reflect.DeepEqual(cold.IDs, warm2.IDs) {
		t.Errorf("IDs drift cold vs warm: cold=%#v warm=%#v", cold.IDs, warm2.IDs)
	}
}

func TestResourceIndexCache_DrawableRoundTripFindingEquivalence(t *testing.T) {
	drawableDir := writeSampleDrawableDir(t)

	prev := ActiveResourceIndexCache()
	SetActiveResourceIndexCache(nil)
	cold := newResourceIndex()
	cold.scanDrawableDir(drawableDir, runtime.NumCPU())
	SetActiveResourceIndexCache(prev)
	...
	warm1 := newResourceIndex()
	warm1.scanDrawableDir(drawableDir, runtime.NumCPU())
	if err := cache.Close(); err != nil {
		t.Fatalf("Close after prime scan: %v", err)
	}
	if s := cache.Stats(); s.Hits != 0 || s.Misses != 1 {
	...
	warm2 := newResourceIndex()
	warm2.scanDrawableDir(drawableDir, runtime.NumCPU())
	if s := cache.Stats(); s.Hits != 1 || s.Misses != 1 {
		t.Errorf("after warm hit: hits=%d misses=%d (want 1/1)", s.Hits, s.Misses)
	}

	if !reflect.DeepEqual(sortStringSlice(cold.Drawables), sortStringSlice(warm2.Drawables)) {
		t.Errorf("Drawables drift cold vs warm: cold=%#v warm=%#v", cold.Drawables, warm2.Drawables)
	}
	if !reflect.DeepEqual(cold.DrawableSelectors, warm2.DrawableSelectors) {
		t.Errorf("DrawableSelectors drift cold vs warm:\n cold=%#v\n warm=%#v", cold.DrawableSelectors, warm2.DrawableSelectors)
	}
}

func TestResourceIndexCache_HasWritesExcludesReadHits(t *testing.T) {
	repo := t.TempDir()
	fingerprint := strings.Repeat("a", 64)
	idx := newResourceIndex()
	idx.Strings["hello"] = "Hello"

	cache, err := NewResourceIndexCache(repo)
	if err != nil {
		t.Fatalf("NewResourceIndexCache: %v", err)
	}
	if err := cache.SaveAsync(fingerprint, ValuesScanAll, "res", idx); err != nil {
		t.Fatalf("SaveAsync: %v", err)
	}
	if !cache.HasWrites() {
		t.Fatal("expected async save to record cache writes")
	}
	if err := cache.Close(); err != nil {
		t.Fatalf("Close: %v", err)
	}

	fresh, err := NewResourceIndexCache(repo)
	if err != nil {
		t.Fatalf("NewResourceIndexCache fresh: %v", err)
	}
	if _, ok := fresh.Load(fingerprint, ValuesScanAll, "res"); !ok {
...
func TestResourceIndexCache_LayoutInvalidatesOnFileChange(t *testing.T) {
	layoutDir := writeSampleLayoutDir(t)
	repo := t.TempDir()
	cache := withResourceCache(t, repo)

	idx := newResourceIndex()
	if err := idx.scanLayoutDir(layoutDir, 2); err != nil {
		t.Fatalf("prime: %v", err)
	}
	if err := cache.Close(); err != nil {
		t.Fatalf("Close after prime: %v", err)
	}
	warm := newResourceIndex()
	if err := warm.scanLayoutDir(layoutDir, 2); err != nil {
		t.Fatalf("warm1: %v", err)
	}
	if s := cache.Stats(); s.Hits != 1 {
		t.Fatalf("warm1 hits: got %d want 1", s.Hits)
	...
	if err := os.WriteFile(filepath.Join(layoutDir, "main.xml"), []byte(changedLayoutXML), 0o644); err != nil {
		t.Fatalf("mutate layout: %v", err)
	}
	after := newResourceIndex()
	if err := after.scanLayoutDir(layoutDir, 2); err != nil {
		t.Fatalf("after-mutate: %v", err)
	}
	if !after.IDs["changed_title"] {
		t.Fatalf("changed layout id missing after mutation; IDs=%#v", after.IDs)
	}
	if after.IDs["title"] {
		t.Fatalf("stale layout id served from cache after mutation; IDs=%#v", after.IDs)
	}
	if s := cache.Stats(); s.Misses != 2 {
		t.Errorf("after mutation expected misses=2 (prime + post-mutate), got %d", s.Misses)
	}
...
func TestResourceIndexCache_DrawableInvalidatesOnSelectorChange(t *testing.T) {
	drawableDir := writeSampleDrawableDir(t)
	repo := t.TempDir()
	cache := withResourceCache(t, repo)

	idx := newResourceIndex()
	idx.scanDrawableDir(drawableDir, runtime.NumCPU())
	if err := cache.Close(); err != nil {
		t.Fatalf("Close after prime: %v", err)
	}
	warm := newResourceIndex()
	warm.scanDrawableDir(drawableDir, runtime.NumCPU())
	if s := cache.Stats(); s.Hits != 1 {
		t.Fatalf("warm1 hits: got %d want 1", s.Hits)
	}

	if err := os.WriteFile(filepath.Join(drawableDir, "button.xml"), []byte(changedDrawableSelectorXML), 0o644); err != nil {
		t.Fatalf("mutate selector: %v", err)
	}
	after := newResourceIndex()
	after.scanDrawableDir(drawableDir, runtime.NumCPU())
	items := after.DrawableSelectors["button"]
	if len(items) == 0 {
		t.Fatalf("selector metadata missing after mutation: %#v", after.DrawableSelectors)
	}
	if _, ok := items[0].StateAttrs["android:state_checked"]; !ok {
		t.Fatalf("changed selector state missing after mutation: %#v", items[0].StateAttrs)
	}
	if _, ok := items[0].StateAttrs["android:state_pressed"]; ok {
		t.Fatalf("stale selector state served from cache after mutation: %#v", items[0].StateAttrs)
	}
	if s := cache.Stats(); s.Misses != 2 {
		t.Errorf("after mutation expected misses=2 (prime + post-mutate), got %d", s.Misses)
	}
...
func TestComputeLayoutDirFingerprint_StableAndSensitive(t *testing.T) {
	f := []layoutDirFile{
		{name: "a", path: "/r/layout/a.xml", content: []byte("a")},
		{name: "b", path: "/r/layout/b.xml", content: []byte("b")},
	}
	base := computeLayoutDirFingerprint("/r/layout", f)
	if base == "" {
		t.Fatal("empty fingerprint")
	}
	if again := computeLayoutDirFingerprint("/r/layout", f); again != base {
		t.Errorf("unstable: %q vs %q", base, again)
	}
	if diff := computeLayoutDirFingerprint("/r/layout-land", f); diff == base {
		t.Error("dir change did not shift fingerprint")
	}
	f2 := []layoutDirFile{
		{name: "a", path: "/r/layout/a.xml", content: []byte("a")},
		{name: "b", path: "/r/layout/b.xml", content: []byte("b2")},
	}
	if diff := computeLayoutDirFingerprint("/r/layout", f2); diff == base {
		t.Error("content change did not shift fingerprint")
	}
	f3 := append(append([]layoutDirFile(nil), f...), layoutDirFile{name: "c", path: "/r/layout/c.xml", content: []byte("c")})
	if diff := computeLayoutDirFingerprint("/r/layout", f3); diff == base {
		t.Error("new file did not shift fingerprint")
	}
	valuesShape := []valuesDirFile{
		{path: "/r/layout/a.xml", content: []byte("a")},
		{path: "/r/layout/b.xml", content: []byte("b")},
	}
	if diff := computeValuesDirFingerprint("/r/layout", ValuesScanNone, valuesShape); diff == base {
		t.Error("layout and values fingerprints should not share key space")
	}
}

func TestComputeDrawableDirFingerprint_StableAndSensitive(t *testing.T) {
	f := []drawableDirFile{
		{name: "a", path: "/r/drawable/a.xml", ext: ".xml", content: []byte("a")},
		{name: "b", path: "/r/drawable/b.png", ext: ".png"},
	}
	base := computeDrawableDirFingerprint("/r/drawable", f)
	if base == "" {
		t.Fatal("empty fingerprint")
	}
	if again := computeDrawableDirFingerprint("/r/drawable", f); again != base {
		t.Errorf("unstable: %q vs %q", base, again)
	}
	if diff := computeDrawableDirFingerprint("/r/drawable-night", f); diff == base {
		t.Error("dir change did not shift fingerprint")
	}
	f2 := []drawableDirFile{
		{name: "a", path: "/r/drawable/a.xml", ext: ".xml", content: []byte("a2")},
		{name: "b", path: "/r/drawable/b.png", ext: ".png"},
	}
	if diff := computeDrawableDirFingerprint("/r/drawable", f2); diff == base {
		t.Error("xml content change did not shift fingerprint")
	}
	f3 := append(append([]drawableDirFile(nil), f...), drawableDirFile{name: "c", path: "/r/drawable/c.webp", ext: ".webp"})
	if diff := computeDrawableDirFingerprint("/r/drawable", f3); diff == base {
		t.Error("new file did not shift fingerprint")
	}
	layoutShape := []layoutDirFile{
		{name: "a", path: "/r/drawable/a.xml", content: []byte("a")},
	}
	if diff := computeLayoutDirFingerprint("/r/drawable", layoutShape); diff == base {
		t.Error("drawable and layout fingerprints should not share key space")
	}
}
