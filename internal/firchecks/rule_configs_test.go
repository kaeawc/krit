...
func TestCheckSendsConfiguredRuleOptionsAsRuleConfigs(t *testing.T) {
	...
	if _, err := d.Check([]fileRef{{Path: "/src/A.kt"}}, nil, nil, []string{"InjectDispatcher"}, ruleConfigs); err != nil {
...
func TestCheckOmitsRuleConfigsWhenNoneConfigured(t *testing.T) {
	...
	if _, err := d.Check(nil, nil, nil, []string{"A"}, RuleConfigs{"A": {}}); err != nil {
		t.Fatalf("Check: %v", err)
	}
...
func TestCheckDecodesEscapedControlCharactersInMessages(t *testing.T) {
	...
	resp, err := d.Check(nil, nil, nil, []string{"InjectDispatcher"}, nil)
	if err != nil {
		t.Fatalf("Check: %v", err)
	}
...
func TestFirFingerprintChangesWithRuleOptions(t *testing.T) {
	...
	first := FirInvocationFingerprint(nil, jar, rules, optsA(1))
	for i := 0; i < 20; i++ { // map iteration order must not matter
		if again := FirInvocationFingerprint(nil, jar, rules, optsA(1)); again != first {
	...
	second := FirInvocationFingerprint(nil, jar, rules, optsA(2))
	if first == second {
	...
	if FirInvocationFingerprint(nil, jar, rules, nil) != FirInvocationFingerprint(nil, jar, rules, RuleConfigs{"A": {}}) {
