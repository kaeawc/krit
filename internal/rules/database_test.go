...
func TestRoomQueryWithLikeMissingEscape_Positive(t *testing.T) {
	findings := runRuleByName(t, "RoomQueryWithLikeMissingEscape", `
package test
...
func TestRoomQueryWithLikeMissingEscape_NegativeWildcardEmbedding(t *testing.T) {
	findings := runRuleByName(t, "RoomQueryWithLikeMissingEscape", "package test\n\nannotation class Dao\nannotation class Query(val value: String)\n\n@Dao\ninterface UserDao {\n    @Query(\"SELECT * FROM users WHERE name LIKE '%' || :q || '%' ESCAPE '\\\\'\")\n    fun search(q: String): List<Int>\n}\n")
	if len(findings) != 0 {
		t.Fatalf("expected no findings, got %d: %v", len(findings), findings)
	}
}

func TestRoomQueryWithLikeMissingEscape_NegativeExactMatch(t *testing.T) {
	findings := runRuleByName(t, "RoomQueryWithLikeMissingEscape", `
package test
...
`)
	if len(findings) != 0 {
		t.Fatalf("expected no findings for non-LIKE, got %d: %v", len(findings), findings)
	}
}
...
func TestRoomFlowQueryWithoutDistinct_Fixtures(t *testing.T) {
	root := fixtureRoot(t)
	...
	t.Run("positive fixture flags caller missing distinctUntilChanged", func(t *testing.T) {
		...
		findings := runParsedFilesRule(t, "RoomFlowQueryWithoutDistinct", file)
		if len(findings) != 1 {
			t.Fatalf("expected 1 finding, got %d: %v", len(findings), findings)
		}
		if !strings.Contains(findings[0].Message, "observeUsers") ||
			!strings.Contains(findings[0].Message, "distinctUntilChanged") {
			t.Fatalf("expected message about observeUsers/distinctUntilChanged, got %q", findings[0].Message)
		}
	})
