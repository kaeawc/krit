...
func TestXMLParseCache_HasWritesExcludesReadHits(t *testing.T) {
	repo := t.TempDir()
	data := largeManifest()
	...
	if err := pc.SaveAsync(data, &XMLNode{Tag: "manifest"}); err != nil {
		t.Fatalf("SaveAsync: %v", err)
	}
	if !pc.HasWrites() {
	...
	fresh, err := NewXMLParseCache(repo)
	if err != nil {
		t.Fatalf("NewXMLParseCache fresh: %v", err)
	}
	if _, ok := fresh.Load(data); !ok {
