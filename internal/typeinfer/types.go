...
// KotlinAutoImports maps simple names of types Kotlin auto-imports from
// java.lang.* (which is implicitly visible in every Kotlin file even
// without an explicit import statement) to their Java FQNs.
//
// These names are only used when no explicit import, alias, primitive,
// or stdlib mapping wins first. Several java.lang.* types are remapped
// by Kotlin to kotlin.* counterparts (String, Throwable, Exception,
// Object, ...) — those live in PrimitiveTypes / KotlinStdlibTypes and
// are not duplicated here. The entries below are types Kotlin uses by
// their java.lang.* FQN.
