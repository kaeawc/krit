...
func DefaultActiveRules() []*api.Rule {
	active := make([]*api.Rule, 0, len(api.Registry))
...
func BuildDispatcher(activeRules []*api.Rule, resolver typeinfer.TypeResolver) *rules.Dispatcher {
	return rules.NewDispatcher(activeRules, resolver)
}
...
// SingleFileAnalyzer wraps the active rule set and a pre-built
...
func NewSingleFileAnalyzer(active []*api.Rule, resolver typeinfer.TypeResolver) *SingleFileAnalyzer {
	if active == nil {
