...
func TestObservablePhase_UsesInjectedNow(t *testing.T) {
	const step = 11 * time.Millisecond
	...
	o := &ObservablePhase[string, int]{Inner: inner, Now: fake.Now}
	...
	if records[0].Duration != step {
		t.Errorf("Duration = %v, want %v", records[0].Duration, step)
	}
}
