...
| 13 | Rule consistency | ✅ shipped | **All 4/4 acceptance criteria met.** All 634 rules on v2; V2Dispatcher handles all per-file dispatch; `legacyRules` slice deleted; `SetModuleIndex`/`SetResolver` calls removed from main.go; all 27 packages pass. Zero occurrences of the 12 legacy family interfaces (`FlatDispatchRule`, `LineRule`, `CrossFileRule`, `ModuleAwareRule`, `ManifestRule`, `ResourceRule`, `GradleRule`, `TypeAwareRule`, `ConfidenceProvider`, `OracleFilterProvider`, `FixLevelRule`, `AggregateRule`, `ParsedFilesRule`) remain in the source tree — type assertions replaced with anonymous interface types; Manifest/Resource/Gradle slices keep their exported `rules.ManifestRules`/`ResourceRules`/`GradleRules` variables typed as `[]ManifestFamily`/`ResourceFamily`/`GradleFamily` anonymous-interface aliases. Signal-Android validates: 102,308 findings in 29s, byte-identical to pre-migration output. Tracking → [`clusters/core-infra/unified-rule-interface.md`](clusters/core-infra/unified-rule-interface.md) |
| 14 | Android-lint stubs | ✅ shipped | all 181 AOSP Android Lint Issue IDs have implementations (35 Manifest + 35 Resource + 13 Gradle + 13 Icon + 77 source-scanning); 2 stubs remain: `ScrollViewCount`, `OnClick`. `DalvikOverride` removed as obsolete (Dalvik gone since Android 5.0). Ongoing tracking → [`roadmap/clusters/android-lint/`](clusters/android-lint/README.md) |
...
| 17 | Depth over breadth | ✅ shipped | 2026-04-16: all phases complete. Phases 1-5 (P0 bugs, dedup, core-pkg unit tests, Confidence field), Phase 6 (stub-count transparency in `--list-rules`), Phase 7 (CI regression gate via `TestRegression_Baselines`) all shipped. 527/527 rules classified with `Confidence()`. 109/109 fixable rules have per-rule fixtures |
...
refs remaining (all in `edge_test.go` stubResolver, not reachable from
live dispatch). `internal/typeinfer/` has zero refs.
