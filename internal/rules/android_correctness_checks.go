...
func (r *WrongViewCastRule) checkKotlin(ctx *api.Context) {
	file := ctx.File
...
func (r *WrongViewCastRule) checkJava(ctx *api.Context) {
	file := ctx.File
	switch file.FlatType(ctx.Idx) {
...
func (r *WrongViewCastRule) checkLookup(ctx *api.Context, call uint32, castType string) {
	file := ctx.File
...
func wrongViewCastAndroidLookupConfidence(ctx *api.Context, call uint32) (float64, bool) {
	file := ctx.File
...
func rangeSummaryForFile(file *scanner.File) *rangeFileSummary {
	...
	return filefacts.FileFact(fileFactsCache(), file, slotRangeSummary, func() *rangeFileSummary {
...
func rangeSpecsForCall(ctx *api.Context, call uint32, callName string, args uint32, summary *rangeFileSummary) []rangeArgSpec {
...
func rangeOracleCallTarget(ctx *api.Context, call uint32) string {
...
func objectAnimatorBindingCall(ctx *api.Context, call uint32) (targetExpr, propertyArg uint32, propertyName string, ok bool) {
	file := ctx.File
	if file == nil || file.FlatType(call) != "call_expression" {
		return 0, 0, "", false
	}
...
func objectAnimatorCallIsObjectAnimator(ctx *api.Context, call uint32) bool {
	file := ctx.File
...
func objectAnimatorOracleCallTarget(ctx *api.Context, call uint32) string {
	if ctx.Resolver == nil || ctx.File == nil {
		return ""
	}
	cr, ok := ctx.Resolver.(*oracle.CompositeResolver)
