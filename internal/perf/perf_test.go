...
func TestNewWithNow_DeterministicDurations(t *testing.T) {
	t.Parallel()

	fake := newFakeClock(time.Date(2026, 5, 1, 0, 0, 0, 0, time.UTC))
	tracker := NewWithNow(fake.Now)
...
func TestNewWithNow_NestedSerialUsesInjectedNow(t *testing.T) {
	t.Parallel()
...
func TestNewWithNow_NilFallsBackToDefault(t *testing.T) {
	t.Parallel()

	tracker := NewWithNow(nil)
	if !tracker.IsEnabled() {
		t.Fatal("NewWithNow(nil) returned a disabled tracker")
	}
