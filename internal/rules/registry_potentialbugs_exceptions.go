...
func registerPotentialbugsExceptionsRules() {
	...
	}
	{
		...
		v2.Register(&v2.Rule{
			ID: r.RuleName, Category: r.RuleSetName, Description: r.Desc, Sev: v2.Severity(r.Sev),
			NodeTypes: []string{"jump_expression", "throw_expression", "throw_statement"}, Needs: v2.NeedsResolver, Implementation: r,
			Check: r.check,
		})
	}
	{
	...
	}
	{
		...
		v2.Register(&v2.Rule{
			ID: r.RuleName, Category: r.RuleSetName, Description: r.Desc, Sev: v2.Severity(r.Sev),
			NodeTypes: []string{"statements"}, Confidence: 0.75, Fix: v2.FixSemantic, Implementation: r,
