	...
	for i, line := range file.Lines {
		...
		if m := rangeSetAlphaRe.FindStringSubmatch(line); m != nil {
			if v := parseInt(m[1]); v < 0 || v > 255 {
				ctx.Emit(r.Finding(file, i+1, 1, "setAlpha() value "+m[1]+" is outside the valid range [0, 255]."))
			}
		}
			...
			for _, arg := range strings.Split(m[2], ",") {
				...
				if rangeNumericRe.MatchString(arg) {
					if v := parseInt(arg); v < 0 || v > 255 {
						ctx.Emit(r.Finding(file, i+1, 1, "Color."+m[1]+"() argument "+arg+" is outside the valid range [0, 255]."))
						break
					}
				}
			}
		...
		if m := rangeSetProgressRe.FindStringSubmatch(line); m != nil {
			if v := parseInt(m[1]); v < 0 || v > 100 {
				ctx.Emit(r.Finding(file, i+1, 1, "setProgress() value "+m[1]+" is outside the valid range [0, 100]."))
			}
		}
		if m := rangeSetRotationRe.FindStringSubmatch(line); m != nil {
			if v := parseFloat(m[1]); v < -360 || v > 360 {
				ctx.Emit(r.Finding(file, i+1, 1, "setRotation() value "+m[1]+" is outside the typical range [-360, 360]."))
			}
		}
	}
}
