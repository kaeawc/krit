...
// config descriptor registry: validate that the inline descriptor data
// on the three licensing rules' api.Register calls produces the SAME
// observable effect as the config adapter in config.go#applyRuleConfig.
// These tests prove the registry contract against real rule structs —
// the config adapter path is deliberately kept; only the descriptor
// path is exercised additionally.

// licensingDescriptor returns the merged RuleDescriptor for a licensing
// rule registered under the given ID. The descriptor data lives on the
// api.Rule literal in registry_licensing.go (after the meta_licensing.go
// migration); MetaForRule walks the registry, falls back to
// MetaProvider when present, and is the production lookup path used by
// LSP / schema / config.
func licensingDescriptor(t *testing.T, id string) api.RuleDescriptor {
	t.Helper()
	for _, r := range api.Registry {
		if r.ID == id {
			d, ok := MetaForRule(r)
			if !ok {
				t.Fatalf("MetaForRule(%q) returned no descriptor", id)
			}
			return d
		}
	}
	t.Fatalf("licensing rule %q not found in api.Registry", id)
...
// TestLicensing_Meta_Descriptors asserts that each licensing rule's
// registered descriptor has the expected shape. This covers identity,
...
func TestLicensing_Meta_Descriptors(t *testing.T) {
	t.Run("CopyrightYearOutdated", func(t *testing.T) {
		d := licensingDescriptor(t, "CopyrightYearOutdated")
	...
	t.Run("MissingSpdxIdentifier", func(t *testing.T) {
		d := licensingDescriptor(t, "MissingSpdxIdentifier")
	...
	t.Run("DependencyLicenseUnknown", func(t *testing.T) {
		d := licensingDescriptor(t, "DependencyLicenseUnknown")
...
// TestLicensing_Meta_RegistryApply exercises api.ApplyConfig against a
...
func TestLicensing_Meta_RegistryApply(t *testing.T) {
	desc := licensingDescriptor(t, "DependencyLicenseUnknown")
	...
	active := api.ApplyConfig(rule, desc, cfg)
	if !active {
	...
	active2 := api.ApplyConfig(rule2, desc, api.NewFakeConfigSource())
	if active2 {
...
// with a *config.Config) and the new registry (api.ApplyConfig with a
...
func TestLicensing_Parity_DependencyLicenseUnknown(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			registryDesc := licensingDescriptor(t, "DependencyLicenseUnknown")
			registryActive := api.ApplyConfig(registryRule, registryDesc, registryCfg)
...
// buildRegistryConfig builds a api.FakeConfigSource matching the
// same test case.
func buildRegistryConfig(tc licensingParityCase) *api.FakeConfigSource {
	cfg := api.NewFakeConfigSource()
	if tc.ruleSetActive != nil {
		...
		t.Run(rr.id, func(t *testing.T) {
			for _, tc := range cases {
				...
				t.Run(tc.name, func(t *testing.T) {
					...
					regCfg := api.NewFakeConfigSource()
					if tc.ruleSetActive != nil {
					...
					regDesc := licensingDescriptor(t, rr.id)
					registryActive := api.ApplyConfig(ruleInstance, regDesc, regCfg)
