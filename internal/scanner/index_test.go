...
// for the FlatFindChild sentinel-collision bug. Before the fix,
...
// lookups for a large share of symbols on representative projects.
...
func BenchmarkBuildCodeIndexFromRefs_LargeScale(b *testing.B) {
	refs := makeLargeScaleRefs(120000, 6700, 5000000)
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkBuildCodeIndexFromRefs_LazyLargeScale(b *testing.B) {
	refs := makeLargeScaleRefs(120000, 6700, 5000000)
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func makeLargeScaleRefs(nameCount, fileCount, refCount int) []Reference {
