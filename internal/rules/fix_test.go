...
func runBundledFixableFixtures(t *testing.T, dir string, activeRules []*api.Rule) (int, error) {
...
func rulesNeedResolver(activeRules []*api.Rule) bool {
	for _, rule := range activeRules {
		if rule != nil && rule.Needs.Has(api.NeedsResolver) {
			return true
		}
	}
	return false
	...
	for _, entry := range entries {
		...
		t.Run("fixable/per-rule/"+name, func(t *testing.T) {
			...
			if rule.Needs.Has(api.NeedsResolver) {
				resolver := typeinfer.NewResolver()
				resolver.IndexFilesParallel([]*scanner.File{file}, 1)
				soloDispatcher = rules.NewDispatcher([]*api.Rule{rule}, resolver)
			}
...
// coverage debt that remains while fix metadata is declared on api.Rule.Fix.
