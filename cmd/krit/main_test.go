...
func TestPerfFlag_ModuleAwareAnalysisDoesNotSpanCrossFileWorkWithoutModules(t *testing.T) {
	dir := writeTempKt(t, "PerfNoModules.kt", "package test\n\nfun main() {\n    println(\"hi\")\n}\n")
	...
	perfArr, ok := result["perfTiming"].([]interface{})
	if !ok || len(perfArr) == 0 {
		t.Fatalf("expected perfTiming to be a non-empty array, got: %v", result["perfTiming"])
	}
	moduleIndex := perfTimingTopLevelIndex(perfArr, "moduleAwareAnalysis")
	crossIndex := perfTimingTopLevelIndex(perfArr, "crossFileAnalysis")
	if moduleIndex < 0 || crossIndex < 0 {
		return
	}
	if moduleIndex > crossIndex {
		t.Fatalf("moduleAwareAnalysis should close before unrelated cross-file work: moduleIndex=%d crossIndex=%d perf=%#v", moduleIndex, crossIndex, perfArr)
	}
	moduleEntry, moduleOK := perfTimingFindName(perfArr, "moduleAwareAnalysis")
	crossEntry, crossOK := perfTimingFindName(perfArr, "crossFileAnalysis")
	if !moduleOK || !crossOK {
		return
	}
	moduleDuration, _ := moduleEntry["durationMs"].(float64)
	crossDuration, _ := crossEntry["durationMs"].(float64)
	if moduleDuration > 0 && crossDuration > 0 && moduleDuration > crossDuration {
		t.Fatalf("moduleAwareAnalysis should not span unrelated cross-file work: module=%v cross=%v perf=%#v", moduleDuration, crossDuration, perfArr)
	}
}

func TestPerfFlag_NoDuplicateTypeIndexWhenResolverIndexSkipped(t *testing.T) {
	dir := writeTempKt(t, "PerfNoTypeIndex.kt", "package test\n\nfun main() {\n    println(\"hi\")\n}\n")
	...
	if count := perfTimingTopLevelCount(perfArr, "typeIndex"); count > 1 {
		t.Fatalf("expected at most one top-level typeIndex timing when resolver indexing is skipped, got %d: %#v", count, perfArr)
	}
}
...
func perfTimingFindName(entries []interface{}, name string) (map[string]interface{}, bool) {
	for _, raw := range entries {
		...
		children, _ := entry["children"].([]interface{})
		if child, ok := perfTimingFindName(children, name); ok {
			return child, true
		}
	}
	return nil, false
...
func perfTimingTopLevelIndex(entries []interface{}, name string) int {
...
func perfTimingTopLevelCount(entries []interface{}, name string) int {
	count := 0
	for _, raw := range entries {
		entry, ok := raw.(map[string]interface{})
		if ok && entry["name"] == name {
