...
func runRuleWithOracle(t *testing.T, code string, filePath string, diags []oracle.Diagnostic) []scanner.Finding {
	t.Helper()
	path := filepath.Join(t.TempDir(), "test.kt")
	if err := os.WriteFile(path, []byte(code), 0644); err != nil {
		t.Fatal(err)
	...
	d := rules.NewDispatcher([]*api.Rule{rule}, composite)
	cols := d.Run(f)
	return cols.Findings()
}
