...
func contactsSameFileConstantIsReadContactsFlat(file *scanner.File, _ uint32, name string) bool {
	if file == nil || name == "" {
		return false
	}
	found := false
...
func contactsSameFileConstantIsBackgroundLocationFlat(file *scanner.File, _ uint32, name string) bool {
	if file == nil || name == "" {
		return false
	}
	found := false
