...
import (
	...
	used "github.com/kaeawc/krit/internal/usedsymbols"
)

type usedSymbolsModuleResult struct {
	Module  string        `json:"module"`
	Symbols []used.Symbol `json:"symbols"`
}
...
func runUsedSymbolsFile(path string, asJSON bool) int {
	...
	if asJSON {
		res := used.FileResult{File: path, Symbols: syms}
		if err := json.NewEncoder(os.Stdout).Encode(res); err != nil {
			fmt.Fprintf(os.Stderr, "error: %v\n", err)
			return 1
		}
...
func aggregateSymbols(files []*scanner.File, isInModule func(string) bool) []used.Symbol {
	seen := make(map[string]used.Symbol)
	for _, f := range files {
		for _, s := range used.Extract(f) {
	...
	out := make([]used.Symbol, 0, len(seen))
	for _, s := range seen {
		out = append(out, s)
	}
