...
func TestCollectOracleFiles_TreeSitterOnly(t *testing.T) {
	...
	rules := []FilterRule{
		{Name: "EmptyCatchBlock", Filter: &FilterSpec{}},
	}
...
func TestCollectOracleFiles_FilteredByIdentifier(t *testing.T) {
	...
	rules := []FilterRule{
		{Name: "RedundantSuspendModifier", Filter: &FilterSpec{Identifiers: []string{"suspend"}}},
	}
...
func TestCollectOracleFiles_AllFilesShortCircuits(t *testing.T) {
	...
	rules := []FilterRule{
		{Name: "Deprecation", Filter: &FilterSpec{AllFiles: true}},
		// An identifier rule that would only match B.kt — ignored because
		// the AllFiles rule above short-circuits.
		{Name: "Filtered", Filter: &FilterSpec{Identifiers: []string{"B"}}},
	}
	got := CollectOracleFiles(rules, []*scanner.File{f1, f2})
	if !got.AllFiles {
...
func TestCollectOracleFiles_UnionOfIdentifierFilters(t *testing.T) {
	...
	rules := []FilterRule{
		{Name: "RedundantSuspendModifier", Filter: &FilterSpec{Identifiers: []string{"suspend"}}},
		{Name: "UnsafeCast", Filter: &FilterSpec{Identifiers: []string{" as "}}},
		{Name: "UnnecessaryNotNullOperator", Filter: &FilterSpec{Identifiers: []string{"!!"}}},
	}
...
func TestCollectOracleFiles_DedupsIdentifiers(t *testing.T) {
	...
	rules := []FilterRule{
		{Name: "R1", Filter: &FilterSpec{Identifiers: []string{"suspend"}}},
		{Name: "R2", Filter: &FilterSpec{Identifiers: []string{"suspend"}}},
	}
	got := CollectOracleFiles(rules, []*scanner.File{f1})
...
func TestWriteFilterListFile_AllFilesReturnsEmpty(t *testing.T) {
	summary := FilterSummary{AllFiles: true, TotalFiles: 10, MarkedFiles: 10}
...
// oracle-fingerprint gate: the fingerprint of the same relative path set
// must be identical regardless of where the repo is checked out on disk.
