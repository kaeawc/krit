...
func TestParseTestFixturesDep(t *testing.T) {
	content := `
dependencies {
  testImplementation(testFixtures(project(":messaging-service")))
}
`
	graph := NewModuleGraph("/tmp")
	deps := parseDeps(content, graph)
	...
	if deps[0].ModulePath != ":messaging-service" {
		t.Errorf("expected :messaging-service, got %q", deps[0].ModulePath)
	}
