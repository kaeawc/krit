...
func TestParseSignalAppDeps(t *testing.T) {
	...
	err = ParseDependencies(graph, app)
	if err != nil {
		t.Fatalf("ParseDependencies: %v", err)
	}

	if len(app.Dependencies) < 10 {
		t.Errorf("expected at least 10 deps for :app, got %d", len(app.Dependencies))
	}
	...
	for _, want := range []string{":core-util", ":libsignal-service", ":billing"} {
		if !found[want] {
			t.Errorf("missing dep %s", want)
		}
	}
...
func TestParseCircuitFoundationDeps(t *testing.T) {
	...
	cf := graph.Modules[":circuit-foundation"]
	if cf == nil {
		t.Fatal("expected :circuit-foundation module")
	}

	err = ParseDependencies(graph, cf)
	if err != nil {
		t.Fatalf("ParseDependencies: %v", err)
	}

	if len(cf.Dependencies) < 5 {
		t.Errorf("expected at least 5 deps for :circuit-foundation, got %d", len(cf.Dependencies))
	}

	// Should have typesafe accessor deps resolved.
	found := make(map[string]bool)
	for _, d := range cf.Dependencies {
		found[d.ModulePath] = true
	}
	for _, want := range []string{":backstack", ":circuit-runtime", ":circuit-retained"} {
		if !found[want] {
			t.Errorf("missing dep %s in circuit-foundation", want)
		}
	}

	// Should detect maven-publish.
	if !cf.IsPublished {
		t.Error("expected circuit-foundation to be published")
	}
}
