#!/usr/bin/env bash
# Krit onboarding — Gum prototype.
#
# Scans a target directory with each shipped profile, shows a
# comparison table, and asks the user to pick one.
# Then it runs the controversial-rules questionnaire, writes the merged
# config, runs an optional safe autofix pass, and optionally writes a baseline.
#
# Usage:
