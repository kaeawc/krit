...
func (r *ConventionPluginDeadCodeRule) check(ctx *api.Context) {
	pmi := ctx.ModuleIndex
...
func scanAppliedConventionPluginIDs(graph *module.Graph) map[string]bool {
...
func gradleScriptsToInspect(graph *module.Graph) []string {
	if graph == nil {
		return nil
	}
...
func hardcodedEnvironmentLiteralFlat(file *scanner.File, arg uint32) string {
	...
	labelPresent := flatValueArgumentLabel(file, arg) != ""
	return hardcodedEnvironmentLiteral(file.FlatNodeText(arg), labelPresent)
}
...
func tryReadSourceModuleGradleInfo(dir string) (sourceModuleGradleInfo, bool) {
	info := filefacts.StringFact(fileFactsCache(), dir, slotSourceModuleGradle, func() sourceModuleGradleInfo {
		return readSourceModuleGradleInfo(dir)
	})
	return info, info.found
}

func readSourceModuleGradleInfo(dir string) sourceModuleGradleInfo {
	var path string
...
func (r *CommentedOutImportRule) checkNode(ctx *api.Context) {
	file := ctx.File
	if !strings.HasSuffix(file.Path, ".kt") && !strings.HasSuffix(file.Path, ".kts") {
		return
	}
...
func (r *MergeConflictMarkerLeftoverRule) check(ctx *api.Context) {
	file := ctx.File
	if !strings.HasSuffix(file.Path, ".kt") && !strings.HasSuffix(file.Path, ".kts") &&
...
func printlnNonProductionPath(path string) bool {
	if scanner.IsTestFile(path) || isGradleBuildScript(path) || strings.HasSuffix(filepath.Base(path), ".kts") {
		return true
	}
...
func printlnInsideGradleTaskAction(file *scanner.File, idx uint32) bool {
	fn, ok := flatEnclosingFunction(file, idx)
	return ok && flatHasAnnotationNamed(file, fn, "TaskAction")
}
...
// AST node directly.
func (r *HardcodedLocalhostURLRule) check(ctx *api.Context) {
	file := ctx.File
	if file.Language != scanner.LangKotlin && file.Language != scanner.LangJava {
		return
	}
	if scanner.IsTestFile(file.Path) || isDebugSourceFile(file.Path) {
		return
	}
	idx := ctx.Idx
...
func (r *TestOnlyImportInProductionRule) check(ctx *api.Context) {
	file := ctx.File
	if !strings.HasSuffix(file.Path, ".kt") && !strings.HasSuffix(file.Path, ".kts") {
		return
	}
...
func (r *TestFixtureAccessedFromProductionRule) check(ctx *api.Context) {
	index := ctx.CodeIndex
	if index == nil {
		return
	}
	...
	for _, file := range parsedFiles {
		if file == nil || isTestFixturePath(file.Path) || scanner.IsTestFile(filepath.ToSlash(file.Path)) {
			continue
		}
	...
	for _, file := range parsedFiles {
		...
		if scanner.IsTestFile(filepath.ToSlash(file.Path)) || isTestFixturePath(file.Path) || isGeneratedSourcePath(file.Path) {
			continue
		}
		...
		fileCtx := &api.Context{File: file, Resolver: ctx.Resolver, CodeIndex: index}
		file.FlatWalkAllNodes(0, func(idx uint32) {
...
func resolveFixtureAccessNode(ctx *api.Context, idx uint32, packageName string, fixtures map[string]testFixtureDeclaration, imports map[string]fixtureImportBinding, importedNames map[string]string, productionDeclarations map[string]bool) (testFixtureDeclaration, float64, bool) {
	if ctx.File == nil {
...
func resolveFixtureCall(ctx *api.Context, call uint32, packageName string, fixtures map[string]testFixtureDeclaration, imports map[string]fixtureImportBinding, importedNames map[string]string, productionDeclarations map[string]bool) (testFixtureDeclaration, float64, bool) {
...
func resolveFixtureNavigation(ctx *api.Context, nav uint32, packageName string, fixtures map[string]testFixtureDeclaration, imports map[string]fixtureImportBinding, importedNames map[string]string, productionDeclarations map[string]bool) (testFixtureDeclaration, float64, bool) {
...
func resolveFixtureTypeLikeNode(ctx *api.Context, idx uint32, packageName string, fixtures map[string]testFixtureDeclaration, imports map[string]fixtureImportBinding, importedNames map[string]string, productionDeclarations map[string]bool) (testFixtureDeclaration, float64, bool) {
...
func resolveFixtureName(ctx *api.Context, name string, ref uint32, packageName string, fixtures map[string]testFixtureDeclaration, imports map[string]fixtureImportBinding, importedNames map[string]string, productionDeclarations map[string]bool) (testFixtureDeclaration, float64, bool) {
	if name == "" {
		return testFixtureDeclaration{}, 0, false
	}
...
func sameFileClassLikeDeclarationShadows(ctx *api.Context, name string, ref uint32) bool {
	if ctx.File == nil || name == "" {
		return false
	}
	file := ctx.File
	var shadowed bool
...
func (r *TestFixtureAccessedFromProductionRule) emitTestFixtureAccess(ctx *api.Context, file *scanner.File, idx uint32, decl testFixtureDeclaration, confidence float64, emitted map[string]bool) {
	line := file.FlatRow(idx) + 1
	col := file.FlatCol(idx) + 1
...
func (r *TimberTreeNotPlantedRule) check(ctx *api.Context) {
	...
	for _, file := range files {
		if file == nil || file.FlatTree == nil || scanner.IsTestFile(file.Path) {
			continue
		}
...
func timberProjectFiles(ctx *api.Context) []*scanner.File {
...
func resolveTimberCall(parent *api.Context, file *scanner.File, call uint32, imports timberImportInfo) (timberCallSite, bool) {
	localCtx := &api.Context{File: file, Resolver: parent.Resolver, CodeIndex: parent.CodeIndex}
...
func timberPlantInApplicationOnCreate(ctx *api.Context, file *scanner.File, call uint32) bool {
...
func timberQualifiedPath(file *scanner.File, idx uint32) string {
	...
	case "navigation_expression":
		...
		return strings.Join(parts, ".")
	case "call_expression":
		target, ok := semantics.ResolveCallTarget(&api.Context{File: file}, idx)
