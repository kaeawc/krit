...
type ParseInput struct {
	...
	// Reporter routes verbose progress and warning lines emitted from
	// the phase. Nil means silent. cmd/ packages construct a Reporter
	// with diag.NewStderr; tests substitute a buffer-backed Reporter.
...
type OutputInput struct {
	...
	Format string
	// BaselinePath, when non-empty, is the path to a baseline file
	// used to suppress known findings.
	BaselinePath string
