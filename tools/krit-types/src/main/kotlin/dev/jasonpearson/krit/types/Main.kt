...
class DaemonSession(
    ...
    @OptIn(KaExperimentalApi::class)
    fun handleDecompileJar(request: DaemonRequest): String {
        val fqn = request.fqn
        if (fqn.isNullOrBlank()) {
            return """{"id":${request.id},"error":"decompileJar requires fqn"}"""
        }
        val classId = ClassId.topLevel(FqName(fqn))
        return try {
            val text = analyze(sourceModule) {
                val symbol = findClass(classId)
                val psi = symbol?.psi
                val fileText = psi?.containingFile?.text
                if (!fileText.isNullOrBlank()) {
                    fileText
                } else if (symbol is KaNamedClassSymbol) {
                    val cls = extractClass(symbol, profile = DeclarationExportProfile.full(), memo = memo)
                    renderClassResultStub(request.jarPath.orEmpty(), fqn, cls)
                } else {
                    null
                }
            } ?: return """{"id":${request.id},"error":"decompileJar could not resolve ${escJsonStr(fqn)}"}"""
            """{"id":${request.id},"result":{"text":${esc(text)}}}"""
        } catch (t: Throwable) {
            """{"id":${request.id},"error":"decompileJar failed: ${escJsonStr(t.message ?: t.javaClass.simpleName)}"}"""
        }
    }
    ...
    /**
     * Resolves the expression type at each requested (file, line, col)
     * position. Used by the targeted-resolution pre-pass under
     * --depth=thorough: rules declare positions of interest; the
     * dispatcher batches them; this method walks only the requested
     * positions and calls KAA's expressionType — no full-file walk.
     *
     * Cost scales with position count, not file size, so the previous
     * 70%-CPU regression from bulk visiting every KtNameReferenceExpression
     * is avoided by construction.
     */
    @OptIn(KaExperimentalApi::class)
    fun handleResolveExpressionTypes(request: DaemonRequest): String {
        val payload = request.expressionPositions
        if (payload.isNullOrEmpty()) {
            return buildResolveExpressionTypesResponse(request.id, emptyMap(), emptyMap())
        }

        val ktFiles = sourceModule.psiRoots.filterIsInstance<KtFile>()
        val results = linkedMapOf<String, Map<String, ResolvedExpressionFact>>()
        val errors = linkedMapOf<String, String>()

        for ((requestedPath, positions) in payload) {
            val resolvedPath = File(requestedPath).canonicalPath
            val ktFile = ktFiles.find { file ->
                file.virtualFilePath == resolvedPath ||
                    file.virtualFilePath.endsWith(requestedPath) ||
            ...
            try {
                results[requestedPath] = resolveExpressionTypesForKtFile(ktFile, positions)
            } catch (e: Exception) {
                errors[requestedPath] = e.message ?: "Resolution failed"
                System.err.println("Error resolving expressions in ${ktFile.virtualFilePath}: ${e.message}")
            }
        }

        return buildResolveExpressionTypesResponse(request.id, results, errors)
    }

    /**
     * For each requested position in ktFile, finds the smallest
     * KtExpression at that offset and resolves its type via KAA.
     * Returns a map keyed by "line:col" mirroring the shape used in
     * Oracle.LookupExpression. Positions that don't resolve to a
     * KtExpression or whose expressionType is null are silently
     * omitted — the Go-side resolver treats absence as "no fact."
     */
    @OptIn(KaExperimentalApi::class)
    private fun resolveExpressionTypesForKtFile(
        ktFile: KtFile,
        positions: List<RequestExpressionPosition>
    ): Map<String, ResolvedExpressionFact> {
        val document = ktFile.viewProvider.document ?: return emptyMap()
        val out = linkedMapOf<String, ResolvedExpressionFact>()
        for (pos in positions) {
            val key = "${pos.line}:${pos.col}"
            val offset = try {
                document.getLineStartOffset(pos.line - 1) + (pos.col - 1)
            } catch (_: IndexOutOfBoundsException) {
                continue
            }
            val expr = ktFile.findElementAt(offset)?.parentOfType<KtExpression>() ?: continue
            try {
                analyze(expr) {
                    val type = expr.expressionType ?: return@analyze
                    val fqn = (type as? KaClassType)?.classId?.asFqNameString()
                        ?: type.toString()
                    val simpleName = fqn.substringAfterLast('.')
                    out[key] = ResolvedExpressionFact(
                        name = simpleName,
                        fqn = fqn,
                ...
                // Per-position failures are silent — KAA can throw on
                // half-resolved bodies and we'd rather return partial
                // results than fail the whole batch. Matches the
                // resilience pattern in analyzeKtFile's call resolver.
            }
        }
        return out
    }
...
/**
 * One resolved expression fact destined for the JSON response. Shape
 * matches what oracle.LookupExpression returns Go-side after parsing
 * (typeinfer.ResolvedType minus Kind, which is recomputed from name).
 */
data class ResolvedExpressionFact(
    val name: String,
    val fqn: String,
...
/**
 * Builds the JSON response for resolveExpressionTypes. Pure function —
 * no KAA, no PSI — so it's testable in isolation. Output shape:
 *
 *   {"id": <id>, "result": {"types": {<file>: {"L:C": {...}}}, "errors": {<file>: <msg>}}}
 *
 * The errors map is omitted when empty so success responses stay flat.
 */
fun buildResolveExpressionTypesResponse(
    id: Long,
    byFile: Map<String, Map<String, ResolvedExpressionFact>>,
    errors: Map<String, String>
): String {
    val sb = StringBuilder()
    sb.append("""{"id":""").append(id).append(""","result":{"types":{""")
    var firstFile = true
    for ((path, byKey) in byFile) {
        if (!firstFile) sb.append(',')
        firstFile = false
        sb.append('"').append(escJsonStr(path)).append("\":{")
        var firstEntry = true
        for ((key, fact) in byKey) {
            if (!firstEntry) sb.append(',')
            firstEntry = false
            sb.append('"').append(key).append("\":{")
            sb.append("\"name\":\"").append(escJsonStr(fact.name)).append('"').append(',')
            sb.append("\"fqn\":\"").append(escJsonStr(fact.fqn)).append('"').append(',')
            sb.append("\"nullable\":").append(fact.nullable)
            sb.append('}')
        }
        sb.append('}')
    }
    ...
    if (errors.isNotEmpty()) {
        ...
        for ((path, msg) in errors) {
            ...
            firstErr = false
            sb.append('"').append(escJsonStr(path)).append("\":\"").append(escJsonStr(msg)).append('"')
        }
        sb.append('}')
    }
    sb.append("}}")
...
data class DaemonRequest(
    ...
    // Targeted-resolution payload for the resolveExpressionTypes method.
    // Maps file path → list of (1-based line, 1-based col) positions
    // whose expression type the caller wants resolved. Null/empty for
    // every other method.
...
data class RequestExpressionPosition(val line: Int, val col: Int)
...
/**
 * Parses the resolveExpressionTypes payload shape:
 *
 *   "expressionPositions": {
 *       "/path/to/file.kt": [{"line": 7, "col": 13}, {"line": 9, "col": 5}]
 *   }
 *
 * Returns null when the key is absent. Returns an empty map when the
 * key is present but the object body is empty. Returns null on any
 * structural mismatch — callers treat that as "no positions to resolve."
 */
@Suppress("ReturnCount")
fun extractJsonExpressionPositionsMap(json: String, key: String): Map<String, List<RequestExpressionPosition>>? {
    val keyPattern = """"$key""""
    ...
    val out = linkedMapOf<String, List<RequestExpressionPosition>>()
    while (i < json.length) {
        ...
        if (i >= json.length || json[i] != ':') return null
        i = skipJsonWhitespace(json, i + 1)
        val arrayResult = parseExpressionPositionArray(json, i) ?: return null
...
@Suppress("ReturnCount")
private fun parseExpressionPositionArray(json: String, start: Int): Pair<List<RequestExpressionPosition>, Int>? {
    var i = skipJsonWhitespace(json, start)
    if (i >= json.length || json[i] != '[') return null
    i++
    val out = mutableListOf<RequestExpressionPosition>()
    while (i < json.length) {
        i = skipJsonWhitespace(json, i)
        if (i < json.length && json[i] == ']') return out to i + 1
        val obj = parseExpressionPositionObject(json, i) ?: return null
        out.add(obj.first)
        i = skipJsonWhitespace(json, obj.second)
        when {
            i < json.length && json[i] == ',' -> i++
...
@Suppress("ReturnCount")
private fun parseExpressionPositionObject(json: String, start: Int): Pair<RequestExpressionPosition, Int>? {
    var i = skipJsonWhitespace(json, start)
    if (i >= json.length || json[i] != '{') return null
    i++
    var line: Int? = null
    ...
    while (i < json.length) {
        ...
        if (i < json.length && json[i] == '}') {
            ...
            return RequestExpressionPosition(line, col) to i
        }
        val keyResult = parseJsonStringLiteral(json, i) ?: return null
        val fieldName = keyResult.first
        i = skipJsonWhitespace(json, keyResult.second)
        if (i >= json.length || json[i] != ':') return null
        i = skipJsonWhitespace(json, i + 1)
        val numResult = parseJsonNonNegativeInt(json, i) ?: return null
        when (fieldName) {
            "line" -> line = numResult.first
            "col" -> col = numResult.first
        }
        i = skipJsonWhitespace(json, numResult.second)
        when {
            i < json.length && json[i] == ',' -> i++
            i < json.length && json[i] == '}' -> Unit
            else -> return null
        }
    }
    return null
...
@Suppress("ReturnCount")
private fun parseJsonNonNegativeInt(json: String, start: Int): Pair<Int, Int>? {
    var i = skipJsonWhitespace(json, start)
    val begin = i
    ...
    val value = json.substring(begin, i).toIntOrNull() ?: return null
    return value to i
}
...
// RFC 8259. Handles all 0x00-0x1F bytes, since Kotlin source strings can
// legitimately contain literal tab / CR / bell / etc. inside test-fixture
// string literals, and emitting those raw produces JSON that strict parsers
// reject with "invalid control character in string literal" at decode time.
...
fun appendClassCompact(sb: StringBuilder, c: ClassResult) {
    ...
    if (c.jarPath != null) sb.append(""""jarPath":${esc(c.jarPath)},""")
...
// possible Kotlin source pattern. Specific files in large repos can trigger
// internal FIR builder bugs like "FirPropertyImpl with Source origin was
// instantiated without a source element" when a function contains `arr[i]++`
// or similar array-increment patterns. These bubble up as
...
fun jarPathOf(symbol: KaSymbol?): String? {
    if (symbol == null || symbol.origin == KaSymbolOrigin.SOURCE) return null
    val psi: PsiElement = symbol.psi ?: return null
    val containing = psi.containingFile ?: return null
    val vf = containing.virtualFile ?: return null
    return normalizeVirtualJarPath(vf.path)
}

fun normalizeVirtualJarPath(path: String): String? {
    var p = path.removePrefix("jar://")
    val bang = p.indexOf("!/")
    if (bang >= 0) p = p.substring(0, bang)
    return if (p.endsWith(".jar")) p else null
}
        ...
        try {
            ...
            if (includeExpressions) {
                ...
                if (document != null) {
                    ...
                    // KtExpression.expressionType on every node, which made
                    // expression collection dominate large-project profiles because
...
// DiagnosticResult holds a compiler diagnostic for JSON export; values are populated
// in analyzeKtFile from KaDiagnosticCheckerFilter.
...
fun appendClass(sb: StringBuilder, c: ClassResult, ind: String) {
    ...
    if (c.jarPath != null) sb.appendLine("$ind  \"jarPath\": ${esc(c.jarPath)},")
...
fun renderClassResultStub(jarPath: String, fqn: String, cls: ClassResult): String {
    val pkg = fqn.substringBeforeLast('.', "")
    val simple = fqn.substringAfterLast('.')
    val b = StringBuilder()
    b.appendLine("// Decompiled stub generated by krit.")
    b.append("// Source: ")
    b.appendLine(if (jarPath.isBlank()) "(unresolved classpath entry)" else jarPath)
    b.appendLine("// Bodies are omitted; signatures only.")
    b.appendLine()
    if (pkg.isNotBlank()) {
    ...
    if (cls.visibility != "public") modifiers.add(cls.visibility)
    if (cls.isAbstract && cls.kind == "class") modifiers.add("abstract")
    if (cls.isOpen && cls.kind == "class") modifiers.add("open")
    if (cls.isData) modifiers.add("data")
    val headerKind = when (cls.kind) {
        "interface" -> "interface"
        "object" -> "object"
        "enum" -> "enum class"
        else -> "class"
    }
    val typeParams = if (cls.typeParameters.isEmpty()) "" else cls.typeParameters.joinToString(", ", "<", ">")
    b.append((modifiers + headerKind).joinToString(" "))
    b.append(" ")
    b.append(simple)
    b.append(typeParams)
    if (cls.supertypes.isNotEmpty()) {
        b.append(" : ")
        b.append(cls.supertypes.joinToString(", "))
    }
    ...
    b.appendLine(" {")
    for (m in cls.members.sortedWith(compareBy<MemberResult> { it.kind }.thenBy { it.name })) {
        b.append("    ")
        if (m.visibility != "public") b.append(m.visibility).append(' ')
        if (m.isAbstract) b.append("abstract ")
        when (m.kind) {
            "property" -> {
                b.append("val ").append(m.name)
                if (m.returnType.isNotBlank()) b.append(": ").append(m.returnType)
                b.appendLine()
            }
            else -> {
                b.append("fun ").append(m.name).append('(')
                b.append(m.params.joinToString(", ") { p -> "${p.name}: ${p.type}${if (p.nullable) "?" else ""}" })
                b.append(')')
                if (m.returnType.isNotBlank()) b.append(": ").append(m.returnType)
                b.appendLine()
            }
