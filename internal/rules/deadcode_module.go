...
// CollectModuleAwareNeeds collapses the requirements for module-aware
...
func (r *ModuleDeadCodeRule) check(ctx *api.Context) {
	pmi := ctx.ModuleIndex
...
func formatModuleDeadCodeMsg(sym scanner.Symbol, modPath, category string) string {
	vis := cases.Title(language.Und).String(sym.Visibility)
	switch category {
	case "truly-dead":
