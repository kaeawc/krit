...
// Err exposes the terminal error captured by Update so callers
// driving tea.NewProgram can surface failures after Run returns.
func (m Model) Err() error { return m.err }

func New(opts onboarding.ScanOptions, reg *onboarding.Registry, target, preset string, acceptAll bool) Model {
...
func (m Model) writeConfigCmd(thresholds []onboarding.ThresholdOverride) tea.Cmd {
	target := m.target
...
func (m Model) writeExplorerCmd(overrides []onboarding.Override) tea.Cmd {
	target := m.target
	repoRoot := m.opts.RepoRoot
	profile := m.selected
	return func() tea.Msg {
...
func (m Model) autofixCmd() tea.Cmd {
...
func (m Model) baselineCmd() tea.Cmd {
	kritBin := m.opts.KritBin
	configPath := m.configPath
	target := m.target
	return func() tea.Msg {
