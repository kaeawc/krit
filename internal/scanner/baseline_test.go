...
func TestFilterByBaseline_FilenameOnlyFallback(t *testing.T) {
	// Baseline has filename-only IDs, but filtering uses basePath.
	baseline := &Baseline{
		ManuallySuppressed: map[string]bool{},
		CurrentIssues: map[string]bool{
...
func TestBaselineIDFilenameOnly(t *testing.T) {
	f := Finding{File: "/project/module/src/Foo.kt", Rule: "MagicNumber", Line: 10, Message: "msg"}

	id := BaselineIDFilenameOnly(f, "fun calc()")
	expected := "MagicNumber:Foo.kt:fun calc()"
	if id != expected {
		t.Errorf("BaselineIDFilenameOnly = %q, want %q", id, expected)
	}
}

func TestBaselineIDFilenameOnlyAt_MatchesFindingAPI(t *testing.T) {
	columns := CollectFindings([]Finding{
		{File: "/project/module/src/Foo.kt", Rule: "MagicNumber", Line: 10, Message: "msg"},
	})

	got := BaselineIDFilenameOnlyAt(&columns, 0, "fun calc()")
	want := BaselineIDFilenameOnly(Finding{
		File:    "/project/module/src/Foo.kt",
		Rule:    "MagicNumber",
		Message: "msg",
	}, "fun calc()")
	...
	if got != want {
		t.Fatalf("BaselineIDFilenameOnlyAt = %q, want %q", got, want)
	}
}
