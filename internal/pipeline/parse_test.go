...
func TestParsePhase_Run_CollectsJavaForJavaSourceRule(t *testing.T) {
	...
	rule := api.FakeRule("ParsePhaseJavaSourceRule", api.WithNodeTypes("class_declaration"))
...
func TestParsePhase_Run_JavaFilesHonorExcludes(t *testing.T) {
	...
	rule := api.FakeRule("ParsePhaseJavaExcludeRule", api.WithNodeTypes("class_declaration"))
	rule.Languages = []scanner.Language{scanner.LangJava}
...
func TestParsePhase_Run_GeneratedJavaFilesSkipped(t *testing.T) {
	...
	rule := api.FakeRule("ParsePhaseGeneratedJavaRule", api.WithNodeTypes("class_declaration"))
	rule.Languages = []scanner.Language{scanner.LangJava}
...
func TestParsePhase_Run_BuildsJavaSuppressionIndex(t *testing.T) {
	...
	rule := api.FakeRule("ParsePhaseJavaSuppressionRule", api.WithNodeTypes("class_declaration"))
	rule.Languages = []scanner.Language{scanner.LangJava}
...
// crossFileRule is a minimal api.Rule that declares NeedsCrossFile. Used by
// the Java-collection tests to exercise the conditional Java parse path.
func crossFileRule() *api.Rule {
	return &api.Rule{
		ID:          "X",
		Description: "t",
		NodeTypes:   nil,
...
func TestParsePhase_Run_CollectsJavaFiles_WhenNeedsCrossFile(t *testing.T) {
	...
	in := ParseInput{
		Paths:       []string{dir},
		ActiveRules: []*api.Rule{crossFileRule()},
	}
	out, err := (ParsePhase{}).Run(context.Background(), in)
	if err != nil {
		t.Fatalf("Run: unexpected error: %v", err)
...
func TestParsePhase_Run_LoggerReceivesVerboseLine(t *testing.T) {
	...
	in := ParseInput{
		Paths:    []string{dir},
		Reporter: &diag.Reporter{Verbose: &buf},
	}
	if _, err := (ParsePhase{}).Run(context.Background(), in); err != nil {
		t.Fatalf("Run: %v", err)
	}
	out := buf.String()
	if !strings.Contains(out, "verbose: Parsed") || !strings.Contains(out, "files in") {
		t.Fatalf("Reporter never received the Parsed-files line. Got: %q", out)
	}
}
