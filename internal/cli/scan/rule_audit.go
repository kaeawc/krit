...
func emitRuleAuditPlain(columns *scanner.FindingColumns, byTarget map[string][]int, orderedTargets []string, experimentsByRule map[string][]string, opts RuleAuditOpts) int {
	...
	for i, target := range orderedTargets {
		...
		printAuditTargetHeader(target, i, multi)
		printAuditSummaryLine(byTarget[target], stats, opts)
		if len(stats) == 0 {
			fmt.Println("No rules match the current filters.")
			continue
		}
		printAuditTable(stats)
		unexp := collectUnexperimented(stats, opts.DetailRules)
		if len(unexp) == 0 {
			fmt.Println("No unexperimented rules fired on this target under the current filter.")
			continue
		}
		printAuditDetails(columns, unexp, target, opts)
...
func printAuditTargetHeader(target string, i int, multi bool) {
	if multi {
		if i > 0 {
			fmt.Println()
...
func printAuditSummaryLine(rows []int, stats []*ruleStats, opts RuleAuditOpts) {
	total := len(rows)
	unexperimented := 0
...
func printAuditTable(stats []*ruleStats) {
...
func collectUnexperimented(stats []*ruleStats, limit int) []*ruleStats {
	var unexp []*ruleStats
	for _, rs := range stats {
		if len(rs.experiments) == 0 {
...
func printAuditDetails(columns *scanner.FindingColumns, unexp []*ruleStats, target string, opts RuleAuditOpts) {
...
func countClusterProperties(columns *scanner.FindingColumns, rows []int) clusterCounts {
	cc := clusterCounts{extCounts: make(map[string]int)}
	for _, row := range rows {
		file := columns.FileAt(row)
...
func computeCluster(columns *scanner.FindingColumns, rows []int) string {
	...
	cc := countClusterProperties(columns, rows)
	total := len(rows)
	...
	if cc.manifestCount >= threshold {
		parts = append(parts, "manifest")
	} else if cc.resCount >= threshold {
		parts = append(parts, "res/")
	}
	if cc.testCount >= threshold {
		parts = append(parts, "test")
	}
	if cc.generatedCount >= threshold {
