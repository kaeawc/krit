...
`
	...
	server.Run()
	// Server.Run returns when stdin EOFs, but the workspace-index
	// goroutine started by handleInitialize keeps writing after that.
	// Wait for it to drain before reading the buffer.
