...
func TestDispatcher_DefersCrossFileAndModuleAwareRules(t *testing.T) {
	...
	crossInvoked := 0
	cross := api.FakeRule("Cross",
		api.WithNeeds(api.NeedsCrossFile),
		api.WithCheck(func(ctx *api.Context) { crossInvoked++ }),
	)
	modInvoked := 0
	mod := api.FakeRule("Module",
	...
	dispatchCount, lineCount, crossCount, moduleCount := dispatcher.Stats()
	if dispatchCount != 0 || lineCount != 0 {
		t.Fatalf("expected no dispatch/line rules, got dispatch=%d line=%d", dispatchCount, lineCount)
	}
	if crossCount != 1 {
...
func TestDispatcher_RunWithStats_TracksRuleBuckets(t *testing.T) {
	...
	rule := api.FakeRule("Dispatch",
		api.WithNodeTypes("integer_literal"),
...
func TestDispatcher_FlatDispatchRuleRunsOnFlatTree(t *testing.T) {
	...
	rule := api.FakeRule("FlatDispatch",
		api.WithNodeTypes("integer_literal"),
		api.WithSeverity(api.SeverityWarning),
		api.WithCheck(func(ctx *api.Context) {
			ctx.EmitAt(int(ctx.Node.StartRow)+1, int(ctx.Node.StartCol)+1, "flat finding")
	...
	dispatchCount, lineCount, crossCount, moduleCount := dispatcher.Stats()
	if dispatchCount != 1 || lineCount != 0 || crossCount != 0 || moduleCount != 0 {
		t.Fatalf("unexpected dispatcher stats: dispatch=%d line=%d cross=%d module=%d",
			dispatchCount, lineCount, crossCount, moduleCount)
	}
}
...
func TestDispatcher_ConfidenceProviderOverridesDefault(t *testing.T) {
	...
	rule := api.FakeRule("Medium",
		api.WithNodeTypes("integer_literal"),
		api.WithSeverity(api.SeverityWarning),
		api.WithConfidence(0.60),
		api.WithCheck(func(ctx *api.Context) {
...
func TestHasV2Implementation_RejectsMissingCheck(t *testing.T) {
	rule := &api.Rule{ID: "MissingCheck", Category: "test", NodeTypes: []string{"call_expression"}}
	if HasV2Implementation(rule) {
...
func TestV2RegistryHasRunnableImplementations(t *testing.T) {
	...
	if len(missing) > 0 {
		t.Fatalf("rule registry contains rules without runnable implementations: %v", missing)
	}
}
