...
func emitImpact(asJSON bool, root string, changed []string, hits map[string]map[string]bool, graph *module.Graph) int {
...
func changedFQNsFromGit(root, gitRef string) ([]string, error) {
	return changedFQNsFromGitWith(proc.Default, root, gitRef)
}

// changedFQNsFromGitWith is the testable form of changedFQNsFromGit:
// the runner is injected so tests can script git output without
// invoking real git.
func changedFQNsFromGitWith(runner proc.Runner, root, gitRef string) ([]string, error) {
	res, err := runner.Run(context.Background(), proc.Cmd{
		Name: "git",
		Args: []string{"diff", "--name-only", gitRef},
		Dir:  root,
	})
	if err != nil {
	...
	if res.ExitCode != 0 {
		return nil, fmt.Errorf("git diff %s: exit %d: %s", gitRef, res.ExitCode, strings.TrimSpace(string(res.Stderr)))
	}
	var fqns []string
	seen := make(map[string]bool)
