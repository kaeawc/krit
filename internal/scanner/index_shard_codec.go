...
func decodeShardHeader(r *codecReader) (bloom []byte, err error) {
	m, err := r.getU32()
	if err != nil {
		return nil, err
	}
...
func decodeShardNameTable(r *codecReader) ([]string, error) {
...
func decodeShardSymbols(r *codecReader, path string, lookup func(uint64) (string, error)) ([]Symbol, error) {
	...
	for i := range syms {
		sym, err := decodeOneSymbol(r, path, lookup)
		if err != nil {
			return nil, err
		}
		syms[i] = sym
...
type symbolStrings struct {
	name, kind, vis, pkg, fqn, owner, sig string
}

func decodeSymbolStrings(r *codecReader, lookup func(uint64) (string, error)) (symbolStrings, uint8, error) {
	nid, err := r.getUvarint()
	if err != nil {
		return symbolStrings{}, 0, err
	}
	...
	return symbolStrings{name, kind, vis, pkg, fqn, owner, sig}, lang, nil
}

func decodeSymbolNumerics(r *codecReader) (arity, line, sb, eb int64, f uint8, err error) {
	if arity, err = r.getVarint(); err != nil {
		return
	}
	if line, err = r.getVarint(); err != nil {
		return
	}
	if sb, err = r.getVarint(); err != nil {
		return
	}
	if eb, err = r.getVarint(); err != nil {
		return
	}
	f, err = r.getU8()
	return
...
func decodeOneSymbol(r *codecReader, path string, lookup func(uint64) (string, error)) (Symbol, error) {
	ss, lang, err := decodeSymbolStrings(r, lookup)
	if err != nil {
		return Symbol{}, err
	}
	arity, line, sb, eb, f, err := decodeSymbolNumerics(r)
	if err != nil {
		return Symbol{}, err
	}
	return Symbol{
...
func decodeShardRefs(r *codecReader, path string, lookup func(uint64) (string, error)) ([]Reference, error) {
...
func decodeShardPayload(data []byte, path string) (*fileShard, error) {
	...
	bloom, err := decodeShardHeader(r)
	if err != nil {
		return nil, err
	}
	names, err := decodeShardNameTable(r)
	if err != nil {
		return nil, err
	}
	...
	syms, err := decodeShardSymbols(r, path, lookup)
	if err != nil {
		return nil, err
	}
	refs, err := decodeShardRefs(r, path, lookup)
	if err != nil {
		return nil, err
	}
	return &fileShard{
