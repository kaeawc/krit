...
func (crossFileShardsRegistered) Clear(_ cacheutil.ClearContext) error {
...
// fileShard.Path on load). Cuts shard-payload disk bytes substantially.
// Pre-v5 blobs are raw gob and the zstd magic check rejects them as a
// per-key miss.
...
//
// Capacity is sized for a large-repository upper bound. The exact
// false-positive rate is a function of total items stored, not shard count.
const (
...
// useful for sparse shards whose raw MarshalBinary is nearly all zeros.
