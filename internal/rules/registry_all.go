// This file owns the v2 rule registry. It is intentionally checked in as
// normal Go source so the registry no longer depends on extraction tooling.
//
// The source banners below preserve the historical grouping from the old
// consolidated registry and make future split-outs straightforward.
...
func registerAllRules() {
	...
	}
	{
		...
		v2.Register(&v2.Rule{
			ID: r.RuleName, Category: r.RuleSetName, Description: r.Desc, Sev: v2.Severity(r.Sev),
			NodeTypes: []string{"conjunction_expression", "disjunction_expression"}, Confidence: 0.75, Fix: v2.FixIdiomatic, OriginalV1: r,
			Check: func(ctx *v2.Context) {
				idx, file := ctx.Idx, ctx.File
