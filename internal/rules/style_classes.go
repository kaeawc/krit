...
func abstractClassCanBeInterfacePropertyCarriesState(file *scanner.File, classIdx uint32, propNode uint32) bool {
	if file == nil || classIdx == 0 || propNode == 0 || file.FlatType(propNode) != "property_declaration" {
		return false
	}
	if enclosing, ok := flatEnclosingAncestor(file, propNode, "class_declaration"); ok && enclosing != classIdx {
		return false
	}
	propText := file.FlatNodeText(propNode)
	return strings.Contains(propText, "=") || file.FlatHasModifier(propNode, "abstract")
}
...
func abstractClassCanBeInterfaceSupertypeCarriesState(file *scanner.File, idx uint32) bool {
	...
	for child := file.FlatFirstChild(idx); child != 0; child = file.FlatNextSib(child) {
		...
		if abstractClassCanBeInterfaceHeaderHasStatefulSupertype(file, idx, child) {
			return true
		}
	}
	return false
...
func abstractClassCanBeInterfaceHeaderHasStatefulSupertype(file *scanner.File, classIdx uint32, headerNode uint32) bool {
	found := false
	abstractClassCanBeInterfaceWalkDelegationSpecifiers(file, headerNode, func(spec uint32) bool {
		if abstractClassCanBeInterfaceDelegationSpecCarriesState(file, classIdx, spec) {
			found = true
			return false
		}
		return true
...
func abstractClassCanBeInterfaceWalkDelegationSpecifiers(file *scanner.File, idx uint32, visit func(uint32) bool) {
	if file == nil || idx == 0 || visit == nil {
		return
	}
	if file.FlatType(idx) == "delegation_specifier" {
		visit(idx)
		return
	}
	for child := file.FlatFirstChild(idx); child != 0; child = file.FlatNextSib(child) {
		...
		if file.FlatType(child) == "delegation_specifier" {
			keepGoing = visit(child)
		} else {
			abstractClassCanBeInterfaceWalkDelegationSpecifiers(file, child, visit)
		}
		if !keepGoing {
			return
		}
...
func abstractClassCanBeInterfaceDelegationSpecCarriesState(file *scanner.File, classIdx uint32, spec uint32) bool {
	if file.FlatHasChildOfType(spec, "constructor_invocation") {
		return true
	}
	if strings.Contains(file.FlatNodeText(spec), "(") {
		return true
	}
	supertypeName := extractSupertypeNameFlat(file, spec)
	if supertypeName == "" {
		return false
	}
	...
	file.FlatWalkNodes(0, "class_declaration", func(classNode uint32) {
		if supertypeHasState || classNode == classIdx {
			return
		}
	...
	return supertypeHasState
}

func abstractClassCanBeInterfaceHasConcreteMember(file *scanner.File, idx uint32) bool {
	if file == nil || idx == 0 || file.FlatType(idx) != "class_declaration" {
		return false
	}
	body, _ := file.FlatFindChild(idx, "class_body")
	...
	file.FlatWalkNodes(body, "function_declaration", func(fn uint32) {
		...
		if nested, ok := flatEnclosingAncestor(file, fn, "class_declaration"); ok && nested != idx {
			return
		}
		if file.FlatHasModifier(fn, "abstract") {
			return
		}
		if functionBody, ok := file.FlatFindChild(fn, "function_body"); ok && functionBody != 0 {
			found = true
		}
	})
	return found
...
func abstractClassCanBeInterfaceHasFrameworkClassAnnotation(file *scanner.File, idx uint32) bool {
	for _, name := range []string{"Module", "AssistedFactory"} {
		if hasAnnotationFlat(file, idx, name) || flatHasAnnotationNamed(file, idx, name) {
			return true
		}
	}
	return false
...
func abstractClassCanBeInterfaceHasDaggerBindMethods(file *scanner.File, idx uint32) bool {
	if file == nil || idx == 0 {
		return false
	}
	body, _ := file.FlatFindChild(idx, "class_body")
	...
	file.FlatWalkNodes(body, "function_declaration", func(fn uint32) {
		...
		if hasAnnotationFlat(file, fn, "Binds") || flatHasAnnotationNamed(file, fn, "Binds") {
			found = true
		}
	})
	return found
...
func dataClassBindingKeyword(file *scanner.File, idx uint32, keyword string) uint32 {
	if file == nil || idx == 0 || keyword == "" {
		return 0
	}
	...
	for child := file.FlatFirstChild(bpk); child != 0; child = file.FlatNextSib(child) {
		if file.FlatType(child) == keyword {
			return child
		}
	}
	return 0
...
func dataClassPropertyBelongsToClass(file *scanner.File, prop uint32, classDecl uint32) bool {
	if file == nil || prop == 0 || classDecl == 0 {
		return false
	}
	for parent, ok := file.FlatParent(prop); ok; parent, ok = file.FlatParent(parent) {
		switch file.FlatType(parent) {
		case "function_declaration", "class_declaration", "object_declaration", "interface_declaration":
			return parent == classDecl
		}
	}
	return false
}
...
// the nested class still has internal visibility.
