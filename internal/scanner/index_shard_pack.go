...
// don't carry dead shard data forever.
...
func (h *packHandle) loadForWrite() error {
	if h.loaded {
		return nil
	}
	data, err := os.ReadFile(h.path)
...
func (h *packHandle) buildItems(latest map[string][]byte) []packKV {
	items := make([]packKV, 0, len(h.index)+len(latest))
	for k, e := range h.index {
		...
		items = append(items, packKV{key: k, data: cp, crc: e.crc})
	}
	for k, blob := range latest {
		items = append(items, packKV{key: k, data: blob, crc: crc32.ChecksumIEEE(blob)})
	}
	return items
}

func buildPackBuffer(items []packKV) ([]byte, map[string]packEntry) {
...
func writePackAtomic(path string, buf []byte, ensureDir bool) error {
	if ensureDir {
...
func (h *packHandle) putMany(writes []encodedShardWrite, ensureDir bool) error {
	...
	items := h.buildItems(latest)
	buf, newIndex := buildPackBuffer(items)

	if err := writePackAtomic(h.path, buf, ensureDir); err != nil {
		return err
	}
	h.data = buf
	h.index = newIndex
...
func (ps *packStore) SaveEncodedShards(writes []encodedShardWrite) error {
	...
	// NOTE: long-lived worker pool with a shared error channel; not the
	// per-item shape errgroup.SetLimit handles cleanly. The first error
	// short-circuits via the buffered errCh; switching to errgroup would
	// require either spawning N goroutines per write group (allocation
	// regression) or threading a separate error sink. Refactor candidate
	// only with a benchmark; raw WaitGroup is correct here.
...
// nolint:unparam // (error) result mirrors decodeShardBlob's signature
// so callers stay symmetric. zstd EncodeAll never errors today, but
// future codec swaps (different compressor, framed checksumming) may
// need to surface failure.
