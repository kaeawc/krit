...
func TestCheckFiles_GitDirtyPathsSkipCleanMetadataChecks(t *testing.T) {
	...
	if err := os.WriteFile(fileA, []byte("fun a() = 1\n"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	c := &Cache{
		...
		Files: map[string]FileEntry{
			absA: {
				Hash:    ComputeFileHash(fileA) + "stale",
				ModTime: 1,
				Size:    1,
			},
			absB: {
				Hash:    "clean-entry-with-stale-metadata",
				ModTime: 1,
				Size:    1,
				Columns: testFindingColumns([]scanner.Finding{{
					File: fileB, Line: 1, Col: 1, Severity: "warning", RuleSet: "test", Rule: "Clean", Message: "cached",
				}}),
			},
		},
	}

	result := c.CheckFiles([]string{fileA, fileB}, "samehash", dir)
	if !result.GitDirtyPathsKnown {
		t.Fatalf("expected git dirty paths to be available")
	}
	if result.GitDirtyPathCount < 1 {
		t.Fatalf("dirty path count = %d, want at least 1", result.GitDirtyPathCount)
	}
	if result.CachedPaths[fileA] {
		t.Fatalf("dirty file should not be a cache hit")
	}
	if !result.CachedPaths[fileB] {
		t.Fatalf("clean tracked file should hit without metadata/hash checks")
	}
	if result.CachedColumns.Len() != 1 {
		t.Fatalf("cached columns = %d, want 1", result.CachedColumns.Len())
	}
	if !c.ShouldSkipFullSaveForSmallDelta(result, 4) {
		t.Fatalf("expected tiny dirty delta to skip full JSON cache save")
	}
}
...
func TestCacheFilePath_DifferentScanPaths(t *testing.T) {
	...
	pathA := FilePath(cacheDir, []string{"/project/src"})
	pathB := FilePath(cacheDir, []string{"/project/test"})
...
func TestCacheFilePath_EmptyCacheDir(t *testing.T) {
	path := FilePath("", []string{"/project/src"})
	if path != "" {
...
func TestCacheFilePath_Deterministic(t *testing.T) {
	...
	path1 := FilePath(cacheDir, paths)
	path2 := FilePath(cacheDir, paths)
...
func TestCacheFilePath_OrderIndependent(t *testing.T) {
	...
	pathA := FilePath(cacheDir, []string{"/project/src", "/project/test"})
	pathB := FilePath(cacheDir, []string{"/project/test", "/project/src"})
...
func TestResolveCacheDir_FallbackToScanPath(t *testing.T) {
	...
	expectedDir := filepath.Join(absDir, ".krit", "cache")
	if dir != expectedDir {
		t.Errorf("expected fallback cache dir %s, got %s", expectedDir, dir)
	}
	if filepath.Dir(filePath) != expectedDir {
		t.Errorf("expected cache file in %s, got %s", expectedDir, filePath)
	}
	if !strings.HasPrefix(filepath.Base(filePath), "krit-") || !strings.HasSuffix(filepath.Base(filePath), ".cache") {
		t.Errorf("expected hashed cache filename, got %s", filepath.Base(filePath))
	}
}
...
func TestResolveCacheDir_FileAsScanPath(t *testing.T) {
	...
	if resolvedDir != expectedDir {
		t.Errorf("expected dir=%s when scan path is a file, got %s", expectedDir, resolvedDir)
	}
	if filepath.Dir(cachePath) != expectedDir {
		t.Errorf("expected cache file in %s, got %s", expectedDir, cachePath)
	}
	if !strings.HasPrefix(filepath.Base(cachePath), "krit-") || !strings.HasSuffix(filepath.Base(cachePath), ".cache") {
		t.Errorf("expected hashed cache filename, got %s", filepath.Base(cachePath))
	}
}
...
func TestResolveCacheDir(t *testing.T) {
	...
	t.Run("no scan paths", func(t *testing.T) {
		...
		expectedDir := filepath.Join(".krit", "cache")
		if dir != expectedDir {
			t.Errorf("expected dir=%s, got %s", expectedDir, dir)
		}
		if filePath != filepath.Join(expectedDir, CacheFileName) {
			t.Errorf("expected filePath=%s, got %s", filepath.Join(expectedDir, CacheFileName), filePath)
		}
	})
}
