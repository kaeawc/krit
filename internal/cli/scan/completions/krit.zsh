...
_krit() {
    _arguments \
        ...
        '--remove-dead-code[Bulk-remove directly fixable dead code findings]' \
        '--all-rules[Enable all rules]' \
        '--no-cache[Disable incremental cache]' \
        '--clear-cache[Delete cache and exit]' \
        '--cache-dir[Override incremental cache directory]:dir:_directories' \
        ...
        '--diff[Only check lines changed since git ref]:ref:' \
