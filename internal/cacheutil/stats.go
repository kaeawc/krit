...
// Backend is the canonical interface every persistent on-disk cache in
// krit should satisfy. It is the union of Registered (Name + Clear)
// and StatsProvider (Stats). New caches should implement Backend so
// the CLI's --verbose stats, --clear-caches path, and global Budget
// report all pick them up uniformly. Caches that genuinely have no
// per-entry stats may implement Registered alone.
...
func pctOfCap(bytes, capacity int64) float64 {
	if capacity <= 0 {
		return 0
	}
	p := float64(bytes) / float64(capacity)
