...
type flatOracleLookup interface {
	...
	LookupDiagnosticsForFlatRange(file *scanner.File, idx uint32) []oracle.Diagnostic
}
...
func evalConst(ctx *api.Context, expr uint32, seen map[uint32]bool) (ConstValue, bool) {
...
func evalSameFileConst(ctx *api.Context, ref uint32, seen map[uint32]bool) (ConstValue, bool) {
	if ctx.File == nil || ref == 0 {
		return ConstValue{}, false
	}
	file := ctx.File
	name := referenceName(file, ref)
...
func evalPrefixConst(ctx *api.Context, idx uint32, seen map[uint32]bool) (ConstValue, bool) {
	file := ctx.File
	sign := int64(1)
...
func permissionGuardsInCondition(ctx *api.Context, cond uint32) []PermissionGuard {
	file := ctx.File
	var guards []PermissionGuard
	file.FlatWalkNodes(cond, "call_expression", func(call uint32) {
		localCtx := &api.Context{File: file, Resolver: ctx.Resolver, CodeIndex: ctx.CodeIndex}
