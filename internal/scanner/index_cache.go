...
func recordCrossFileDisk(paths crossFileCachePaths, entries int) {
	...
	crossFileBytes.Store(diskBytes)
...
// bytes times many rows); interning collapses that to uint32 indexes
// and a de-duplicated string list.
...
func fingerprintCrossFileEntries(entries []fingerprintEntry) string {
...
type symStrings struct {
	name, kind, vis, file, pkg, fqn, owner, sig string
}

func lookupSymStrings(getStr func(uint32) (string, bool), cols *packedSymbols, i int) (symStrings, bool) {
	name, ok1 := getStr(cols.Name[i])
	kind, ok2 := getStr(cols.Kind[i])
	vis, ok3 := getStr(cols.Visibility[i])
	file, ok4 := getStr(cols.File[i])
	pkg, ok5 := getStr(cols.Package[i])
	fqn, ok6 := getStr(cols.FQN[i])
	owner, ok7 := getStr(cols.Owner[i])
	sig, ok8 := getStr(cols.Signature[i])
	if !ok1 || !ok2 || !ok3 || !ok4 || !ok5 || !ok6 || !ok7 || !ok8 {
		return symStrings{}, false
	}
	return symStrings{name, kind, vis, file, pkg, fqn, owner, sig}, true
}

func buildSymbol(s symStrings, cols *packedSymbols, i int) Symbol {
	f := cols.Flags[i]
	return Symbol{
		Name:       s.name,
		Kind:       s.kind,
		Visibility: s.vis,
		File:       s.file,
		Line:       int(cols.Line[i]),
		StartByte:  int(cols.StartByte[i]),
		EndByte:    int(cols.EndByte[i]),
		Language:   Language(cols.Language[i]),
		Package:    s.pkg,
		FQN:        s.fqn,
		Owner:      s.owner,
		Signature:  s.sig,
		Arity:      int(cols.Arity[i]),
		IsOverride: f&1 != 0,
		IsTest:     f&2 != 0,
		IsMain:     f&4 != 0,
		IsStatic:   f&8 != 0,
...
func (p cachePayload) unpackSymbols(getStr func(uint32) (string, bool)) ([]Symbol, bool) {
	cols := &p.Syms
	n := len(cols.Name)
	if len(cols.Kind) != n || len(cols.Visibility) != n || len(cols.File) != n ||
		len(cols.Line) != n || len(cols.StartByte) != n || len(cols.EndByte) != n ||
		len(cols.Language) != n || len(cols.Package) != n || len(cols.FQN) != n ||
		len(cols.Owner) != n || len(cols.Signature) != n || len(cols.Arity) != n ||
		len(cols.Flags) != n {
		return nil, false
	}
	symbols := make([]Symbol, n)
	for i := 0; i < n; i++ {
		s, ok := lookupSymStrings(getStr, cols, i)
		if !ok {
			return nil, false
		}
		symbols[i] = buildSymbol(s, cols, i)
	}
	return symbols, true
}

func (p cachePayload) unpackRefs(getStr func(uint32) (string, bool)) ([]Reference, bool) {
	m := len(p.Refs.Name)
...
func (p cachePayload) unpack() ([]Symbol, []Reference, bool) {
	...
	symbols, ok := p.unpackSymbols(getStr)
	if !ok {
		return nil, nil, false
	}
	refs, ok := p.unpackRefs(getStr)
	if !ok {
		return nil, nil, false
	}
...
func (p cachePayload) unpackSymsByName(idx *CodeIndex, symbols []Symbol, getStr func(uint32) (string, bool)) bool {
...
func (p cachePayload) unpackRefCounts(idx *CodeIndex, getStr func(uint32) (string, bool)) bool {
...
func (p cachePayload) unpackRefFiles(idx *CodeIndex, getStr func(uint32) (string, bool)) bool {
...
func (p cachePayload) unpackNCRefFiles(idx *CodeIndex, getStr func(uint32) (string, bool)) bool {
...
func (p cachePayload) unpackNCRefCounts(idx *CodeIndex, getStr func(uint32) (string, bool)) bool {
...
func (p cachePayload) unpackFull() (*CodeIndex, bool) {
	...
	if !p.unpackSymsByName(idx, symbols, getStr) {
		return nil, false
	}
	if !p.unpackRefCounts(idx, getStr) {
		return nil, false
	}
	if !p.unpackRefFiles(idx, getStr) {
		return nil, false
	}
	if !p.unpackNCRefFiles(idx, getStr) {
		return nil, false
	}
	if !p.unpackNCRefCounts(idx, getStr) {
		return nil, false
	}
...
func loadCrossFileCacheMeta(dir string) (CrossFileCacheMeta, crossFileCachePaths, bool) {
	paths := crossFileCacheFiles(dir)
	metaBytes, err := os.ReadFile(paths.Meta)
	if err != nil {
		return CrossFileCacheMeta{}, paths, false
	}
	var meta CrossFileCacheMeta
	if err := json.Unmarshal(metaBytes, &meta); err != nil {
		return CrossFileCacheMeta{}, paths, false
	}
	if meta.Version != CrossFileCacheVersion {
		return CrossFileCacheMeta{}, paths, false
	}
	return meta, paths, true
}

func loadCrossFileCachePayload(paths crossFileCachePaths) (*CodeIndex, bool) {
...
func crossFilePayloadEntries(meta CrossFileCacheMeta) []fingerprintEntry {
	if len(meta.PayloadEntries) > 0 {
		return meta.PayloadEntries
	}
	return meta.Entries
}

func loadCrossFileOverlayEntries(dir string, entries []fingerprintEntry) ([]Symbol, []Reference, bool) {
	if len(entries) == 0 {
		return nil, nil, true
	}
	store := newPackStore(dir)
	var symbols []Symbol
	var refs []Reference
	for _, e := range entries {
		shard, ok := store.LoadShard(e.Path, e.Hash)
		if !ok {
			return nil, nil, false
		}
		symbols = append(symbols, shard.Symbols...)
		refs = append(refs, shard.References...)
	}
	return symbols, refs, true
}

func loadCrossFileCacheIndexFromMeta(dir string, meta CrossFileCacheMeta, paths crossFileCachePaths) (*CodeIndex, bool) {
	idx, ok := loadCrossFileCachePayload(paths)
	if !ok {
		return nil, false
	}
	if len(meta.RemovedPayloadPaths) > 0 || len(meta.OverlayEntries) > 0 {
		removePaths := make(map[string]bool, len(meta.RemovedPayloadPaths))
		for _, p := range meta.RemovedPayloadPaths {
			removePaths[p] = true
		}
		symbols, refs, ok := loadCrossFileOverlayEntries(dir, meta.OverlayEntries)
		if !ok {
			return nil, false
		}
		idx = BuildIndexIncremental(idx, removePaths, symbols, refs)
	}
	return idx, true
}

// LoadCurrentCrossFileCacheIndex returns the current cached index and meta
// without requiring the caller to already know the cached fingerprint.
func LoadCurrentCrossFileCacheIndex(dir string) (*CodeIndex, CrossFileCacheMeta, bool) {
	if dir == "" {
		return nil, CrossFileCacheMeta{}, false
	}
	meta, paths, ok := loadCrossFileCacheMeta(dir)
	if !ok || meta.Fingerprint == "" {
		return nil, CrossFileCacheMeta{}, false
	}
	idx, ok := loadCrossFileCacheIndexFromMeta(dir, meta, paths)
	if !ok {
		return nil, CrossFileCacheMeta{}, false
	}
...
// LoadCurrentCrossFileCacheMeta returns the current cache metadata without
// reading or decoding the full index payload.
func LoadCurrentCrossFileCacheMeta(dir string) (CrossFileCacheMeta, bool) {
	if dir == "" {
		return CrossFileCacheMeta{}, false
	}
	meta, _, ok := loadCrossFileCacheMeta(dir)
	if !ok || meta.Fingerprint == "" {
		return CrossFileCacheMeta{}, false
	}
	return meta, true
...
func LoadCrossFileCacheIndex(dir, wantFingerprint string) (*CodeIndex, bool) {
	...
	meta, paths, ok := loadCrossFileCacheMeta(dir)
	if !ok || meta.Fingerprint != wantFingerprint {
		crossFileMisses.Add(1)
		return nil, false
	}
	idx, ok := loadCrossFileCacheIndexFromMeta(dir, meta, paths)
...
func SaveCrossFileCacheIndex(dir, fingerprint string, meta CrossFileCacheMeta, idx *CodeIndex) error {
	...
	if len(meta.PayloadEntries) == 0 && len(meta.Entries) > 0 {
		meta.PayloadEntries = append([]fingerprintEntry(nil), meta.Entries...)
	}
	meta.OverlayEntries = nil
	meta.RemovedPayloadPaths = nil
	if meta.WrittenAt.IsZero() {
...
// SaveCrossFileCacheOverlay persists only metadata for a small incremental
// update. The existing payload.gob remains the compacted base; overlay
// entries point at per-file shards that replace or extend that base.
func SaveCrossFileCacheOverlay(dir, fingerprint string, meta CrossFileCacheMeta, idx *CodeIndex) error {
	...
	if len(meta.PayloadEntries) == 0 {
		return fmt.Errorf("empty payload entries")
	}
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return fmt.Errorf("mkdir cache dir: %w", err)
	}
	paths := crossFileCacheFiles(dir)
	if _, err := os.Stat(paths.Symbols); err != nil {
		return fmt.Errorf("stat cross-file payload: %w", err)
	}
