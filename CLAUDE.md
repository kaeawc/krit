...
can opt into JVM-backed Kotlin Analysis API/FIR helper facts when source
...
- New rules use the v2 pipeline: implement the local rule struct with the
  existing bases, expose dispatch metadata in the checked-in registry, and
  declare the capabilities the dispatcher must provide.
...
Run all of these before pushing. CI mirrors this sequence and will fail on
any step that fails locally.

```bash
go build -o krit ./cmd/krit/   # Build binary
go vet ./...                    # Lint Go code
golangci-lint run ./...         # Linter suite (gofmt, gocyclo, etc.)
go test ./... -count=1          # Run all tests
make integration                # Full integration suite (build + playground + CLI/LSP/MCP + all packages)
```
...
**Always run `make integration` before pushing.** It catches issues (missing
binary outputs, LSP/MCP regressions, integration-only test failures) that
`go test ./...` alone does not exercise.
