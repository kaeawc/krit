...
- `internal/rules/` - rule implementations, bases, metadata, v2 dispatcher, registry tests.
...
The dispatcher classifies `v2.Rule` entries by `NodeTypes`, `Languages`, and `Needs` capabilities. Node-targeted rules receive matching flat AST nodes during one file walk; line rules scan `file.Lines`; cross-file, module-aware, parsed-file, Android, Gradle, and aggregate rules run in pipeline phases after the needed indexes are built.
...
Config loads `krit.yml` / `.krit.yml` from the project root, with `--config FILE` as an override. The schema and rule descriptors come from checked-in `Meta()` implementations.
