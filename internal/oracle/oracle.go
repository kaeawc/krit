...
type Oracle struct {
	...
	callTargetSuspend     map[string]map[uint64]bool                    // file path → (packed line:col → suspend callable marker)
...
func (o *Oracle) indexFileExpressions(path string, file *File) {
	...
	for pos, et := range file.Expressions {
		...
		if et.CallTargetResolved {
			if ctrMap == nil {
				ctrMap = make(map[uint64]bool)
			}
			ctrMap[key] = true
			if et.CallTargetSuspend {
				if ctsMap == nil {
					ctsMap = make(map[uint64]bool)
				}
				ctsMap[key] = true
			}
		}
	...
	if ctrMap != nil {
		o.callTargetResolved[path] = ctrMap
	}
	if ctsMap != nil {
		o.callTargetSuspend[path] = ctsMap
	}
	if ctaMap != nil {
...
// indexFiles indexes declarations and expressions for all source files.
func (o *Oracle) indexFiles(files map[string]*File) {
	for path, file := range files {
		...
		o.indexFileExpressions(path, file)
	}
}

// freeRawFileData zeroes per-file declaration and expression slices to reduce
// memory after indexing. Dependencies and Diagnostics are intentionally retained.
func (o *Oracle) freeRawFileData() {
...
func Load(path string) (*Oracle, error) {
	...
	o := newOracleShell(&raw)

	for fqn, cls := range raw.Dependencies {
		o.indexClass(fqn, cls)
	}

	o.indexFiles(raw.Files)

	o.buildSupertypeMap()

	o.index = BuildIndex(o.raw)
...
// LoadFromData builds an Oracle from an already-parsed Data struct
...
func (o *Oracle) indexClass(fqn string, cls *Class) {
...
func convertClassInfo(fqn string, cls *Class) *typeinfer.ClassInfo {
...
// suspend at a specific source position. ok is false when the target is absent
...
func (o *Oracle) LookupDiagnostics(filePath string) []Diagnostic {
	...
	result := make([]Diagnostic, len(file.Diagnostics))
