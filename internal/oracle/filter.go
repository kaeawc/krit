...
// FilterRule is a minimal view of a rule that declared
// NeedsOracle. The caller (internal/rules.BuildFilterRulesV2)
...
// FilterSpec is an in-package mirror of api.OracleFilter. The
// value types are decoupled so internal/oracle does not import
// internal/rules. The fields match api.OracleFilter 1:1.
type FilterSpec struct {
	Identifiers []string
	AllFiles    bool
}

// FilterSummary describes the outcome of a filter evaluation.
...
// oracle access on. Rules are represented by FilterRule (the caller
// is expected to build this slice from api.Registry via
...
func CollectOracleFiles(rules []FilterRule, files []*scanner.File) FilterSummary {
	summary := FilterSummary{TotalFiles: len(files)}
